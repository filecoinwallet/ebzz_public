...
//                *canMine=mc_gState->m_Permissions->CanMine(NULL,pubkey_hash);
                ...
                *canMine=mc_gState->m_Permissions->CanMine(NULL,&pubkey_hash);
                if((*canMine & MC_PTP_MINE) == 0)
                {
                    ...
                    {
                        LogPrintf("mchn: _ebzz_ebzzMiner: cannot mine now, waiting...\n");
                    }
                    testValidity=false;
                }
                else
                {
                    ...
                    {
                        LogPrintf("CreateNewBlock(): total size %u\n", nBlockSize);
                        LogPrintf("mchn: _ebzz_ebzzMiner: Starting mining...\n");
                    }                    
                }
            }            
        }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("_ebzz_ebzzMiner : mining is paused, generated block is dropped");            
        }
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
        {
            return error("_ebzz_ebzzMiner : generated block is stale");
        }
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("_ebzz_ebzzMiner : ProcessNewBlock, block not accepted");
...
double GetMinerAndExpectedMiningStartTime(CWallet *pwallet,CPubKey *lpkMiner,set <CTxDestination> *lpsMinerPool,double *lpdMiningStartTime,double *lpdActiveMiners,uint256 *lphLastBlockHash,int *lpnMemPoolSize,double wAvBlockTime)
{
    ...
    *lpdMiningStartTime=mc_TimeNowAsDouble() + 0.5 * dTargetSpacing;        
    ...
    if(!fWaitingForMiner)
    {
        ...
        {
            C_ebzz_ebzzAddress addr=C_ebzz_ebzzAddress(kThisMiner.GetID());
...
void static _ebzz_ebzzMiner(CWallet *pwallet)
{
    LogPrintf("_ebzz_ebzzMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("_ebzz_ebzz-miner");
...
/* MCHN START */            
            ...
            if (Params().MiningRequiresPeers() 
                    ...
                    || (mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz() == 0)
                    )
                    ) {
...
/* MCHN END */    
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in _ebzz_ebzzMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce,pwallet);

            LogPrint("mcminer","mchn-miner: Running _ebzz_ebzzMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        if(mc_gState->m_ProtocolVersionToUpgrade > mc_gState->m_NetworkParams->ProtocolVersion())
                        if( (mc_gState->m_ProtocolVersionToUpgrade > 0) && (mc_gState->IsSupported(mc_gState->m_ProtocolVersionToUpgrade) == 0) )
                        {
                            LogPrintf("_ebzz_ebzzMiner: Waiting for upgrade, block is dropped\n");
                        }
                        else
                        {
...
/* MCHN END */    
    ...
    {
        LogPrintf("_ebzz_ebzzMiner terminated\n");
        throw;
    }
}

void Generate_ebzz_ebzzs(bool fGenerate, CWallet* pwallet, int nThreads)
{
...
/* MCHN END */        
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&_ebzz_ebzzMiner, pwallet));
}
