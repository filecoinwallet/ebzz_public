...
std::string HelpMessage_Cold()
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "_ebzz_ebzz.conf") + "\n";
#if !defined(WIN32)
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "_ebzz_ebzz.pid") + "\n";
#endif
#if !defined(WIN32)
    strUsage += "  -shortoutput           " + _("Returns connection string if this node can start or default bzz address otherwise") + "\n";
#endif
...
/* MCHN END */    
    ...
    strUsage += "  -initprivkey=<privkey>                   " + _("Manually set the wallet default address and private key when running _ebzz_ebzzd for the first time.") + "\n";
    strUsage += "  -hideknownopdrops=<n>                    " + strprintf(_("Remove recognized bzz OP_DROP metadata from the responses to JSON_RPC calls (default: %u)"), 0) + "\n";
...
/* MCHN END */    
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. bzz Core is shutting down."));
...
// Anyway, we don't need this lock as we have permission db lock    
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. bzz Core is probably already running."), strDataDir));
#endif
/* MCHN END */
    ...
    if (!fDisableWallet) {
        ...
        switch(currentwalletdatversion)
        {
            ...
            case 2:
                break;
            case 1:
                return InitError(strprintf("Wallet version 1 is not supported in this version of bzz. "
                        "To upgrade to version 2, run _ebzz_ebzz 1.0: \n"
                        "bzz %s -walletdbversion=2 -rescan\n",mc_gState->m_NetworkParams->Name()));                                        
            default:
...
//            if (!CWalletDB::Recover(bitdbwrap, strWalletFile, true))
            if(!WalletDBRecover(bitdbwrap,strWalletFile,true))
                return false;
            sprintf(bufOutput,"\nTo work properly with salvaged addresses, you have to call importaddress API and restart _ebzz_ebzz with -rescan\n\n");
...
/* MCHN START*/    
    uiInterface.InitMessage(_("Initializing _ebzz_ebzz..."));
    ...
    if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
    {
        ...
        {
            seed_error=strprintf("ERROR: Couldn't initialize asset database for blockchain %s. Please restart _ebzz_ebzzd with reindex=1.\n",mc_gState->m_Params->NetworkName());
        ...
        else
        {
            ...
            if(mc_gState->m_WalletMode != MC_WMD_NONE)
            {
                ...
                {
                    return InitError("Wallet tx database corrupted. Please restart _ebzz_ebzzd with -rescan\n");                        
                }
                ...
                {
                    return InitError(strprintf("Wallet tx database was created with different wallet version (%d). Please restart _ebzz_ebzzd with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_WalletVersion));                        
                }        

                if((pwalletTxsMain->m_Database->m_DBStat.m_InitMode & MC_WMD_MODE_MASK) != (mc_gState->m_WalletMode & MC_WMD_MODE_MASK))
                {
                    return InitError(strprintf("Wallet tx database was created in different mode (%08X). Please restart _ebzz_ebzzd with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_InitMode));                        
                }        
            }
        }
        ...
        if(pwalletMain == NULL)                                                 // Opening wallet only after _ebzz_ebzz parameters were initizalized
        {
            ...
            if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // MCHN-TODO wallet recovery
            {
                ...
                return InitError(_("wallet.dat corrupted. Please try running bzz with -salvagewallet."));                            
            }
...
/*
    ...
    LogPrintf("_ebzz_ebzz protocol version: %d\n",version);
...
/* MCHN START */                
                bool fRet = uiInterface.ThreadSafeMessageBox(
                    strLoadError + ".\n\n" + _("Please restart _ebzz_ebzzd with reindex=1."),
...
*/
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of _ebzz_ebzz Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)                         // MCHN
            {
                strErrors << _("Wallet needed to be rewritten: restart _ebzz_ebzz Core to complete") << "\n";
