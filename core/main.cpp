...
bool Process_ebzz_ebzzRelay(CNode* pfrom, CDataStream& vRecv, CValidationState &state);
bool Process_ebzz_ebzzVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag);
bool Push_ebzz_ebzzVerack(CNode* pfrom, bool fIsVerackack);
bool _ebzz_ebzzNode_CanConnect(CNode *pnode);
bool _ebzz_ebzzNode_DisconnectRemote(CNode *pnode);
bool _ebzz_ebzzNode_DisconnectLocal(CNode *pnode);
bool _ebzz_ebzzNode_RespondToGetData(CNode *pnode);
bool _ebzz_ebzzNode_SendInv(CNode *pnode);
bool _ebzz_ebzzNode_AcceptData(CNode *pnode);
bool _ebzz_ebzzNode_IgnoreIncoming(CNode *pnode);
bool _ebzz_ebzzNode_IsLocal(CNode *pnode);
bool _ebzz_ebzzNode_CollectChunks();
...
#if defined(NDEBUG)
# error "_ebzz_ebzz cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "bzz Signed Message:\n";
...
int SetUpgradedParamValue(const mc_One_ebzz_ebzzParam *param,int64_t value)
{
    ...
    if(strcmp(param->m_Name,"targetblocktime") == 0)
    {
        ...
        Set_ebzz_ebzzParam("targetblocktime",value);
    }   
...
int _ebzz_ebzzNode_ApplyUpgrades(int current_height)
{
    ...
    Set_ebzz_ebzzParams();            
    if(p_ebzz_ebzzFilterEngine)                                                 // Added from version 20012. 
    {
        p_ebzz_ebzzFilterEngine->SetCallbackNames();
    }
...
void _ebzz_ebzzNode_UpdateBlockByHeightList(CBlockIndex *pindex)
{
    if(pindex->nHeight < 0)
    {
        return;
...
bool _ebzz_ebzzNode_IsBlockChainSynced(CNode *pnode)
{
...
/* MCHN START */
        ...
        if(!Accept_ebzz_ebzzTransaction(tx,view,-1,MC_AMT_DEFAULT,reason, &mandatory_fee, &replay))
        {
            return state.DoS(0,
                             error("AcceptToMemoryPool: : Accept_ebzz_ebzzTransaction failed %s : %s", hash.ToString(),reason),
                             REJECT_NONSTANDARD, reason);
        }
...
*/
        ...
        CAmount nFees = 0;
        int async_count=0;
        bool fIsLicenseTokenTransfer=Is__ebzz_ebzzLicenseTokenTransfer(tx);
...
void ThreadScriptCheck() {
    RenameThread("_ebzz_ebzz-scriptch");
...
/* MCHN START */        
    if(!fJustCheck)
    {
        ...
        {
            p_ebzz_ebzzFilterEngine->Reset(pindex->nHeight-1,1);
        }
    }
/* MCHN END */        
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            const CTransaction &tx = block.vtx[i];
            string reason;
            if(!Accept_ebzz_ebzzTransaction(tx,view,offset,MC_AMT_DEFAULT,reason,NULL,NULL))
            {
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                ...
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : Accept_ebzz_ebzzTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
                }
            }
/* MCHN END */                    
...
/* MCHN START */        
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(!Accept_ebzz_ebzzTransaction(tx,view,coinbase_offset,MC_AMT_DEFAULT,reason,NULL,NULL))
                {
                    return state.DoS(0,
                                     error("ConnectBlock: : Accept_ebzz_ebzzTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                     REJECT_INVALID, reason);
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    {
        C_ebzz_ebzzAddress addr=C_ebzz_ebzzAddress(chainActive.Tip()->kMiner.GetID());
...
/* MCHN START */    
    ...
    if(p_ebzz_ebzzFilterEngine)
    {
        p_ebzz_ebzzFilterEngine->Reset(old_height-1,0);
    }
    
    _ebzz_ebzzNode_ApplyUpgrades(old_height-1);        
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
...
/*    
    if(p_ebzz_ebzzFilterEngine)
    {
        p_ebzz_ebzzFilterEngine->Reset(pindexNew->nHeight-1,1);
    }
 */ 
    ...
    }
    {
        ...
        if(p_ebzz_ebzzFilterEngine)
        {
            p_ebzz_ebzzFilterEngine->m_CoinsCache=&view;
        }
        bool rv = ConnectBlock(*pblock, state, pindexNew, view);        
        if(p_ebzz_ebzzFilterEngine)
        {
            p_ebzz_ebzzFilterEngine->m_CoinsCache=NULL;
        }
        if(rv)
        {
...
/* MCHN START */        
    ...
    _ebzz_ebzzNode_ApplyUpgrades(chainActive.Height());    
/* MCHN END */    
...
//    if (!fInvalidFound)
    {
        ...
        if(p_ebzz_ebzzFilterEngine)
        {
            p_ebzz_ebzzFilterEngine->Reset(chainActive.Height(),0);
        }
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    _ebzz_ebzzNode_UpdateBlockByHeightList(pindexNew);
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex, int node_id, CBlockIndex *pindexChecked)
{
    ...
    if( (node_id > 0) && (pindexPrev != NULL) )
    {
        ...
        {
            _ebzz_ebzzNode_UpdateBlockByHeightList(pindexPrev);
        }
        lpNextForPrev=NULL;
...
/* MCHN START */
    ...
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
        ...
        if(pwalletTxsMain->RollBack(NULL,chainActive.Height()) != MC_ERR_NOERROR)
        {
            ...
            {
                strError="Error: The wallet database is inconsistent. Restart bzz with -rescan to rebuild the wallet database from the blockchain (can take hours), or with -skipwalletchecks to continue operating anyway";
                return false;
            }
        }
    }
...
/* MCHN END */        
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            if(p_ebzz_ebzzFilterEngine)
            {
                p_ebzz_ebzzFilterEngine->m_CoinsCache=&coins;
            }            
...
/* MCHN START */    
    if(!_ebzz_ebzzNode_RespondToGetData(pfrom)) 
    {
...
//    RandAddSeedPerfmon();
    ...
    if (strCommand == "version")
    {
        ...
        pfrom->n_ebzz_ebzzServices=0;
        if (!vRecv.empty())
        {
            vRecv >> pfrom->n_ebzz_ebzzServices;
        }
...
/* MCHN START */
        ...
        if(mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz())
        {            
            if(GetBoolArg("-_ebzz_ebzzstylehandshake", false))
            {
                LogPrintf("mchn: _ebzz_ebzz-style-handshake, sending empty verack to peer %d... \n", pfrom->id);
                pfrom->fParameterSetVerified=true;
                pfrom->PushMessage("verack");                
            }
            else
            {
                Push_ebzz_ebzzVerack(pfrom,false);
            }
        }
        else
        {
...
/* MCHN START */
        if(mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz())
        {
            if(vRecv.size() == 0)
            {
                ...
                else
                {
                    ...
                    {
                        LogPrintf("mchn: _ebzz_ebzz-style verack received from peer %d, parameter set NOT VERIFIED, connecting... \n", pfrom->id);
                        pfrom->fParameterSetVerified=true;                    
                        CompleteProcessVersion(pfrom);                
                    }
                }
            ...
            else
            {
                if(GetBoolArg("-_ebzz_ebzzstylehandshake", false))
                {
                    LogPrintf("mchn: _ebzz_ebzz-style-handshake, ignoring verack from peer %d \n", pfrom->id);                    
                }
                else
                {
                    bool disconnect_flag=false;
                    if(!Process_ebzz_ebzzVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !Push_ebzz_ebzzVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz())
        {
            ...
            if(!Process_ebzz_ebzzVerack(pfrom,vRecv,true,&disconnect_flag))
            {
            ...
            {
                pfrom->fDisconnect |= !Push_ebzz_ebzzVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;
...
/* MCHN END */
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!_ebzz_ebzzNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !_ebzz_ebzzNode_IgnoreIncoming(pfrom))      // MCHN
    ...
    else if (strCommand == "tx" && _ebzz_ebzzNode_AcceptData(pfrom))            // MCHN
    {
/* MCHN START */        
        if(!_ebzz_ebzzNode_IgnoreIncoming(pfrom))
        {
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // MCHN
    else if (strCommand == "block" && !fImporting && !fReindex && _ebzz_ebzzNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* MCHN START */        
...
/* MCHN END */        
    ...
    else if (strCommand == "mempool" && _ebzz_ebzzNode_SendInv(pfrom))          // MCHN
    {
...
/* MCHN START */    
    {        
        ...
        if(!pfrom->fDisconnect)
        {
            if(_ebzz_ebzzNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
                    LogPrintf("mchn: Address %s lost connect permission on peer=%d, diconnecting...\n",C_ebzz_ebzzAddress(pfrom->kAddrRemote).ToString().c_str(), pfrom->id);            

            }
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && _ebzz_ebzzNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
                        LogPrintf("mchn: Local address %s lost connect permission. disconnecting peer %d...\n",C_ebzz_ebzzAddress(pfrom->kAddrLocal).ToString().c_str(), pfrom->id);            
                }
            }
        }
    }
...
/* MCHN END */            
        ...
        {
/* MCHN START */            
            if(pfrom->fDisconnect || !_ebzz_ebzzNode_DisconnectRemote(pfrom))
            {
/* MCHN END */            
...
*/
                ...
                if (pto->setInventoryKnown.insert(inv).second)
                {
                    ...
                    if (vInv.size() >= 1000)
                    {
                        if(_ebzz_ebzzNode_SendInv(pto))                         // MCNN
...
/* MCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=_ebzz_ebzzNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
/* MCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && _ebzz_ebzzNode_IsBlockChainSynced(pto))
            {
...
/* MCHN START */        
        ...
        if(pwalletTxsMain->m_ChunkCollector)
        {
            ...
            {
                if(_ebzz_ebzzNode_CollectChunks())
                {
                    _ebzz_ebzzCollectChunks(pwalletTxsMain->m_ChunkCollector);
                }                
                pwalletTxsMain->m_ChunkCollector->m_NextTryTimestamp=time_millis_now+_ebzz_ebzzCollectChunksQueueStats(pwalletTxsMain->m_ChunkCollector);
