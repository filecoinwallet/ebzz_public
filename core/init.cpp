...
mc__ebzz_ebzzFilterEngine* p_ebzz_ebzzFilterEngine = NULL;
...
void Shutdown()
{
    ...
    RenameThread("_ebzz_ebzz-shutoff");
    mempool.AddTransactionsUpdated(1);
    StopRPCThreads();
#ifdef ENABLE_WALLET
    if (pwalletMain)
        bitdbwrap.Flush(false);
    Generate_ebzz_ebzzs(false, NULL, 0);
#endif
    StopNode();
...
/* MCHN START */  
    ...
    {
        delete p_ebzz_ebzzFilterEngine;
        p_ebzz_ebzzFilterEngine=NULL;        
    }
...
std::string HelpMessage(HelpMessageMode mode)                                   // MCHN
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "bzz.conf") + "\n";
    if (mode == HMM__ebzz_ebzzD)
    {
#if !defined(WIN32)
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "bzz.pid") + "\n";
#endif
    strUsage += "  -reindex               " + _("Rebuild the blockchain and reindex transactions on startup.") + "\n";
#if !defined(WIN32)
    strUsage += "  -shortoutput           " + _("Returns connection string if this node can start or default _ebzz-bzz address otherwise") + "\n";
#endif
/* MCHN START */    
/* Default was 0 */    
...
//    strUsage += "  -blockprioritysize=<n> " + strprintf(_("Set maximum size of high-priority/low-fee transactions in bytes (default: %d)"), DEFAULT_BLOCK_PRIORITY_SIZE) + "\n";
    ...
    strUsage += "\n" + _("bzz runtime parameters") + "\n";    
    strUsage += "  -offline                                 " + _("Start _ebzz-bzzd in offline mode, no connections to other nodes.") + "\n";
    strUsage += "  -storeruntimeparams                      " + _("Permanently save modifications to runtime parameters made by setruntimeparam APIs.") + "\n";
    strUsage += "  -initprivkey=<privkey>                   " + _("Manually set the wallet default address and private key when running _ebzz-bzz for the first time.") + "\n";
    strUsage += "  -handshakelocal=<address>                " + _("Manually override the wallet address which is used for handshaking with other peers in a _ebzz-bzz blockchain.") + "\n";
    ...
    strUsage += "  -explorersupport=0|1                     " + _("Provide support for _ebzz-bzz Explorer, default 0") + "\n";

    strUsage += "\n" + _("BZZ API response parameters") + "\n";        
    strUsage += "  -hideknownopdrops      " + strprintf(_("Remove recognized _ebzz-bzz OP_DROP metadata from the responses to JSON-RPC calls (default: %u)"), 0) + "\n";
    ...
    strUsage += "  -v1apicompatible       " + strprintf(_("JSON-RPC calls responses compatible with _ebzz-bzz 1.0 (default: %u)"), 0) + "\n";
...
{
    return FormatParagraph(_("Copyright (c) _ebzz Sciences Ltd - www.bzz.ceo")) + "\n" +
           "\n" +
           ...
           "\n" +
           FormatParagraph(_("Full terms are shown at: http://www.bzz.ceo/service/")) +
           "\n";
    
/*    
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The _ebzz_ebzz Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("_ebzz_ebzz-loadblk");
...
/** Sanity checks
 *  Ensure that _ebzz_ebzz is running in a usable environment with all
        ...
        if(pwalletMain)
        {
            if(pwalletMain->vchDefaultKey.IsValid())
            {
                LogPrintf("mchn: Minimal blockchain parameter set is created, default address: %s\n",
                        C_ebzz_ebzzAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                ...
                {    
                    ...
                    sprintf(bufOutput,"bzz-cli %s grant %s connect\n",mc_gState->m_NetworkParams->Name(),
                         C_ebzz_ebzzAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                    write(OutputPipe,bufOutput,strlen(bufOutput));
                    sprintf(bufOutput,"bzz-cli %s grant %s connect,send,receive\n\n",mc_gState->m_NetworkParams->Name(),
                         C_ebzz_ebzzAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                    write(OutputPipe,bufOutput,strlen(bufOutput));
                }
                else
...
/* MCHN END */    
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. _ebzz-bzz Core is shutting down."));
...
#endif
    // Make sure only a single _ebzz_ebzz process is using the data directory.
...
// Anyway, we don't need this lock as we have permission db lock    
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. _ebzz-bzz Core is probably already running."), strDataDir));
#endif
/* MCHN END */
...
//#endif
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
//    LogPrintf("_ebzz_ebzz version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz())
    {
        LogPrintf("BZZ version build %s protocol %s (%s)\n", mc_BuildDescription(mc_gState->GetNumericVersion()), mc_gState->GetProtocolVersion(), CLIENT_DATE);
    }
    ...
    if (!fDisableWallet) {
        ...
        if (filesystem::exists(pathWalletDat))
        {
            ...
            if(currentwalletdatversion != 1)
            {
                ...
                {
                    return InitError(strprintf("Wallet version %d is not supported in this edition of _ebzz-bzz.\n"
                            "To upgrade to version %d, run _ebzz-Bzz Offline Daemon: \n"
                            "_ebzz-bzz-cold %s -datadir=%s -walletdbversion=3\n"
                            "and restart _ebzz-Bzz or use Community Edition.\n",
        ...
        switch(currentwalletdatversion)
        {
            ...
            case 2:
                break;
            case 1:
                return InitError(strprintf("Wallet version 1 is not supported in this version of _ebzz-Bzz. "
                        "To upgrade to version 2, run _ebzz-Bzz 1.0: \n"
                        "_ebzz-bzz %s -walletdbversion=2 -rescan\n",mc_gState->m_NetworkParams->Name()));                                        
            default:
...
//            if (!CWalletDB::Recover(bitdbwrap, strWalletFile, true))
            if(!WalletDBRecover(bitdbwrap,strWalletFile,true))
                return false;
            sprintf(bufOutput,"\nTo work properly with salvaged addresses, you have to call importaddress API and restart _ebzz-Bzz with -rescan\n\n");
...
//        uiInterface.InitMessage.connect(SetRPCWarmupStatus);
    ...
    uiInterface.InitMessage(_("Initializing _ebzz-Bzz.............."));
    ...
    {    
        ...
        if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // MCHN-TODO wallet recovery
        {
            ...
            return InitError(_("wallet.dat is partially corrupted. Please try running _ebzz-Bzz with -salvagewallet."));                            
        }
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_MINIMAL)
            {
                ...
                if(init_privkey.size())
                {
                    if(seed_attempt == 1)
                    {
                        ...
                        {
                            return InitError(_("The initprivkey runtime parameter can only be used when connecting to _ebzz-Bzz 1.0  2 or later"));                                                        
                        }
...
*/
            ...
            if(mc_QuerySeed(seedThreadGroup,seed_resolved.c_str()))
            {
                ...
                {
                    seed_error="Couldn't disconnect from the seed node, please restart _ebzz-Bzz";
...
//                    if(seed_port.size() == 0)
                    ...
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d - please check _ebzz-Bzz is running at that address and that your firewall settings allow incoming connections.",                
                            seed_ip.c_str(),seed_port);
                        pNodeStatus->sLastError="Couldn't connect to the seed node, _ebzz-Bzz is not running or firewall issue";
                    }
                }
            }
            else
            ...
            if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
            {
                ...
                if(mc_gState->IsSupported(protocol_version) == 0) 
                {
                    ...
                    {
                        seed_error=strprintf("The protocol version (%d) for blockchain %s has been deprecated and was last supported in _ebzz-Bzz %s\n",                
                    ...
                    else
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %d.\n"
                                    "Blockchain was created by _ebzz-Bzz with newer protocol version (%d)\n"                
                                    "Please upgrade to the latest version of _ebzz-Bzz or connect only to blockchains using protocol version %d or earlier.\n",                
            ...
            if(mc_gState->m_NetworkState == MC_NTS_SEED_NO_PARAMS)
            {
                ...
                seed_error=strprintf("Couldn't retrieve blockchain parameters from the seed node %s on port %d.\n"
                            "For _ebzz_ebzz protocol blockchains, the file %s must be copied manually from an existing node.",                
            ...
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
            {
                Select_ebzz_ebzzParams(mc_gState->m_Params->NetworkName());
        ...
        else
        {
            if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_GENERATED)
            {
                ...
                mc_gState->m_NetworkParams->SetGlobals();                           // Needed to update IsProtocol_ebzz_ebzz flag in case of _ebzz_ebzz
    ...
    if(mc_gState->m_NetworkParams->m_Status == MC_PRM_STATUS_VALID)
    {
        ...
        {
            seed_error=strprintf("ERROR: Couldn't initialize asset database for blockchain %s. Please restart _ebzz-Bzz with reindex=1.\n",mc_gState->m_Params->NetworkName());
...
*/
                ...
                {
                    return InitError("Wallet tx database corrupted. Please restart _ebzz-Bzz with -rescan\n");                        
                }
                ...
                {                    
                    ...
                    {
                        return InitError("Explorer database was initialized with different setting. To change it, please restart _ebzz-Bzz with -rescan\n");                                                    
                    }
                }
                ...
                {
                    return InitError(strprintf("Wallet tx database was created with different wallet version (%d). Please restart _ebzz-Bzz with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_WalletVersion));                        
                }        
                ...
                {
                    return InitError(strprintf("Wallet tx database was created in different mode (%08X). Please restart _ebzz-Bzzd with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_InitMode));                        
                }        
            }
        }
        ...
        if(pwalletMain == NULL)                                                 // Opening wallet only after _ebzz_ebzz parameters were initizalized
        {
            pwalletMain = new CWallet(strWalletFile);
            DBErrors nLoadWalletRetForBuild = pwalletMain->LoadWallet(fFirstRunForBuild);
            ...
            if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // MCHN-TODO wallet recovery
            {
                ...
                return InitError(_("wallet.dat corrupted. Please try running _ebzz-Bzzd with -salvagewallet."));                            
            }
        ...
        if(pwalletMain)
        {
            ...
            if(mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz())
            {
                ...
                {
                    LogPrint("mchn","mchn: Default connect  address: %s\n",C_ebzz_ebzzAddress(pkey.GetID()).ToString().c_str());                
                }
...
/* MCHN START */    
    ...
    p_ebzz_ebzzFilterEngine=new mc__ebzz_ebzzFilterEngine;
    if(p_ebzz_ebzzFilterEngine->Initialize())
    {
    ...
    if(!GetBoolArg("-shortoutput", false))
    {
        ...
        {
            sprintf(bufOutput,"BZZ p2p network connection address::\n");
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            sprintf(bufOutput,"_ebzz-bzz %s:%d\n\n",_ebzz_ebzzServerAddress(false).c_str(),GetListenPort());
            bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
            if(found_ips > 1)
            {
                sprintf(bufOutput,"The current network::::::::::::\n");
                bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
        ...
        else
        {
            if(GetBoolArg("-offline",false))
            {                
                sprintf(bufOutput,"BZZ started in offline mode, other nodes cannot connect.\n\n");
                bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));            
            }
            else
            {
    ...
    {
        sprintf(bufOutput,"%s:%d\n",_ebzz_ebzzServerAddress(true).c_str(),GetListenPort());                
        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
    }
...
/* MCHN START */                
                bool fRet = uiInterface.ThreadSafeMessageBox(
                    strLoadError + ".\n\n" + _("Please restart _ebzz-Bzzd with reindex=1."),
...
//    int version=mc_gState->m_NetworkParams->GetInt64Param("protocolversion");
    ...
    LogPrintf("BZZ protocol version: %d\n",version);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of _ebzz-Bzz Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)                         // MCHN
            {
                strErrors << _("Wallet needed to be rewritten: restart _ebzz-Bzz Core to complete") << "\n";
...
//    RandAddSeedPerfmon();
    ...
    // Generate coins in the background
        if (pwalletMain)
            Generate_ebzz_ebzzs(GetBoolArg("-gen", true), pwalletMain, GetArg("-genproclimit", 1));
#endif
...
/* MCHN START */    
    if(!GetBoolArg("-shortoutput", false))
    {    
        sprintf(bufOutput,"_ebzz agreement Node in the operation of the network ready:::::::::::::.\n\n");
        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
    }
    mc_InitRPCHelpMap();
