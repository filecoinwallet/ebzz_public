...
#include "_ebzz_ebzz/_ebzz_ebzz.h"                                              // MCHN
...
/* MCHN START */                            
// Ignoring arguments not starting with "-". original _ebzz_ebzz code discard everything after that                
...
#else
    const char* pszModule = "_ebzz_ebzz";
#endif
    if (pex)
        return strprintf(
...
/* MCHN START */
static boost::filesystem::path pathCached_ebzz_ebzz;
static boost::filesystem::path pathCached_ebzz_ebzzLog;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Mac: ~/Library/Application Support/_ebzz_ebzz
    // Unix: ~/._ebzz_ebzz
/* MCHN START */
    LOCK(csPathCached);
    
    pathCached_ebzz_ebzz=fs::path(string(mc_gState->m_Params->DataDir()));
    fs::path &path =pathCached_ebzz_ebzz;
    return path;
...
/* MCHN START */
    fs::path &path =pathCached_ebzz_ebzzLog;
    if (!path.empty())
        return path;
    path=fs::path(string(mc_gState->m_Params->DataDir(2,1)));
    return pathCached_ebzz_ebzzLog;
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No _ebzz_ebzz.conf file is OK
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "_ebzzbzz.pid"));
