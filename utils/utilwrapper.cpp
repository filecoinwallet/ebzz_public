...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\_ebzz_ebzz
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\_ebzz_ebzz
    // Mac and Unix: ~/._ebzz_ebzz
#ifdef WIN32
    // Windows
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return GetSpecialFolderPath(CSIDL_APPDATA) / "_ebzz_ebzzCold";
    }
    return GetSpecialFolderPath(CSIDL_APPDATA) / "_ebzz_ebzz";
#else
    // Mac and Unix
    ...
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return pathRet / "._ebzzbzz-cold";        
    }
    return pathRet / "._ebzzbzz";
#endif
}
...
void mc_CheckDataDirInConfFile()
{
    ...
    mapConfig=new mc_MapStringString;
    if(mc_ReadGeneralConfigFile(mapConfig,NULL,"_ebzzbzz",".conf") == 0)
    {
                    ...
                    const char *prefix)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override _ebzz_ebzz.conf
                    ...
                    const char *prefix)
{
    try
    {
        ...
        if (!streamConfig.good())
            return MC_ERR_NOERROR; // No _ebzz_ebzz.conf file is OK
...
int mc_Params::ReadConfig(const char *network_name)
{
    ...
    mc_ReadConfigFile(mc_GetConfigFile(network_name,"_ebzzbzz",".conf"),&mapArgs, &mapMultiArgs,"-");    
...
int mc__ebzz_ebzzParams::SetProtocolGlobals()
{
...
int mc__ebzz_ebzzParams::SetGlobals()
{
    m_IsProtocol_ebzz_ebzz=1;
    void *ptr=GetParam("chainprotocol",NULL);
    if(ptr)
    {
        if(strcmp((char*)ptr,"_ebzz_ebzz"))
        {
            m_IsProtocol_ebzz_ebzz=0;
        }
    }
...
int mc__ebzz_ebzzParams::Import(const char *name,const char *source_address)
{
    
    return MC_ERR_NOERROR;
}

std::string _ebzz_ebzzServerAddress(bool check_external_ip)
{
