...
bool Parse_ebzz_ebzzTxOutToBuffer(uint256 hash,                                 // IN, tx hash, if !=0 genesis asset reference is retrieved from asset DB
...
bool Parse_ebzz_ebzzTxOutToBuffer(uint256 hash,                                 // IN, tx hash, if !=0 genesis asset reference is retrieved from asset DB
                                  ...
                                  string& strFailReason)                        // OUT error
{
    return Parse_ebzz_ebzzTxOutToBuffer(hash,txout,amounts,lpScript,allowed,required,NULL,strFailReason);
}
...
bool AddressCanReceive(CTxDestination address)
{
    ...
    C_ebzz_ebzzAddress addressPrint;
    if(lpKeyID != NULL)
    {
        addressPrint=C_ebzz_ebzzAddress(*lpKeyID);
        ptr=(unsigned char*)(lpKeyID);
    }
    else
    {
        addressPrint=C_ebzz_ebzzAddress(*lpScriptID);
