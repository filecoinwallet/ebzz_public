...
bool C_ebzz_ebzzAddressFromTxEntity(C_ebzz_ebzzAddress &address,mc_TxEntity *lpEntity);
Object AddressEntry(C_ebzz_ebzzAddress& address,uint32_t verbose);
...
//    obj.push_back(Pair("protocolversion", PROTOCOL_VERSION));
        obj.push_back(Pair("balance_bzz",       ValueFromAmount(pwalletMain->GetBalance())));
  ...
  //  obj.push_back(Pair("edition", pEF->ENT_Edition()));
   ...
   // obj.push_back(Pair("description", string((char*)mc_gState->m_NetworkParams->GetParam("chaindescription",NULL))));
    ...
    obj.push_back(Pair("nodeaddress", _ebzz_ebzzServerAddress(true) + strprintf(":%d",GetListenPort())));       
 ...
 */ 
    ...
    keyID=pkey.GetID();    
    obj.push_back(Pair("handshakelocal",GetArg("-handshakelocal",C_ebzz_ebzzAddress(keyID).ToString())));   
...
Value setruntimeparam(const json_spirit::Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "This API is supported only if protocol=_ebzz_ebzz");                
    }
...
Value validateaddress(const Array& params, bool fHelp)
{
    ...
    C_ebzz_ebzzAddress address(str);
...
/* MCHN START */        
    if(!isValid)
    {
        ...
        if(isValid)
        {
            address=C_ebzz_ebzzAddress(pubkey.GetID());
        }        
    }
...
Value createkeypairs(const Array& params, bool fHelp)
{
    ...
    for(int i=0;i<count;i++)
    {
        ...
        Object entry;
        entry.push_back(Pair("address", C_ebzz_ebzzAddress(pubkey.GetID()).ToString()));
        entry.push_back(Pair("pubkey", HexStr(pubkey)));
        entry.push_back(Pair("privkey", C_ebzz_ebzzSecret(secret).ToString()));
...
#ifdef ENABLE_WALLET
        // Case 1: _ebzz_ebzz address and we have full public key:
        C_ebzz_ebzzAddress address(ks);
...
/* MCHN END */    
    ...
    C_ebzz_ebzzAddress address(innerID);
