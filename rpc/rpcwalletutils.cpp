...
vector<CTxDestination> ParseAddresses(string param, bool create_full_list, bool allow_scripthash)
{
    ...
    else
    {
        ...
        {
            C_ebzz_ebzzAddress address(tok);
            if (!address.IsValid())
    ...
    else
    {
        ...
        BOOST_FOREACH(const CTxDestination& dest, fromaddresses)
        {
            ...
            if(filter != ISMINE_NO)
            {
                if (!(fIsMine & filter))
                {
                    C_ebzz_ebzzAddress address(dest);
                    throw JSONRPCError(RPC_WALLET_ADDRESS_NOT_FOUND, "Non-wallet address: "+address.ToString());                                                                    
                }
            }
            setAddresses.insert(C_ebzz_ebzzAddress(dest).ToString());
        }
    }
...
bool C_ebzz_ebzzAddressFromTxEntity(C_ebzz_ebzzAddress &address,mc_TxEntity *lpEntity)
{
    ...
    if(is_address)
    {
        address=C_ebzz_ebzzAddress(dest);
        return true;
    }    
    return false;
}
            ...
            if(ptr)
            {
                if( (sighash_type == SIGHASH_ALL) || ( (sighash_type == SIGHASH_SINGLE) && (i == stream_output) ) )
                {
                    ...
                    if(publishers_set.count(publisher_hash) == 0)
                    {
                        ...
                        if(is_redeem_script)
                        {
                            publisher_str=C_ebzz_ebzzAddress((CScriptID)publisher_hash).ToString();
                        }
                        else
                        {
                            publisher_str=C_ebzz_ebzzAddress((CKeyID)publisher_hash).ToString();                    
                        }
                        if(given_publisher)
                        {
...
//            if( AvailableFromStatus(retrieve_status) && ((retrieve_status & MC_OST_STORAGE_MASK) == MC_OST_OFF_CHAIN ))
            if( AvailableFromStatus(retrieve_status) )
            {
                ...
                {        
                    ...
                    if(p_ebzz_ebzzFilterEngine->RunStreamFilters(wtx,stream_output,(unsigned char *)stream_id,filter_block,filter_offset, 
...
//    int iad=-1;
    ...
    if(fIsFound)
    {
        ...
        int required=0;
        if(Parse_ebzz_ebzzTxOutToBuffer(hash,txout,amounts,lpScript,NULL,&required,strFailReason))
        {
                              ...
                              string *strError)
{
    ...
    {
        *strError="Offchain data is not supported with this wallet version. To get this functionality, run \"bzz -walletdbversion=2 -rescan\"";
