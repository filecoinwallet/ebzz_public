...
//        int period_pos=permission_type.find_last_of(".",permission_type.size());
    ...
    for(int i=0;i<permissions->GetCount();i++)
    {
        ...
        if(flags & MC_PFL_IS_SCRIPTHASH)
        {
            ...
            entry.push_back(Pair("address", C_ebzz_ebzzAddress(lpScriptID).ToString()));                
        ...
        else
        {
            ...
            entry.push_back(Pair("address", C_ebzz_ebzzAddress(lpKeyID).ToString()));
        }
...
Value listminers(const Array& params, bool fHelp)
{
    ...
    for(int i=0;i<(int)miners.size();i++)
    {
        ...
        Object entry;
        entry.push_back(Pair("address",C_ebzz_ebzzAddress((CKeyID)(miners[i].m_Address)).ToString()));
