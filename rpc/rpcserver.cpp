...
/* MCHN END */        
    if (strRet == "")
        strRet = strprintf("For more information help: http://help.bzz.ceo: %s\n", strCommand);
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "bzz server stopping _After stop may be on your network to mount";
}
...
void StartRPCThreads(string& strError)
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use _ebzz_ebzzd, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
              "It is recommended you use the following random password:\n"
              "rpcuser=_ebzz_ebzzrpc\n"
              "rpcpassword=%s\n"
              "(you do not need to remember this password)\n"
              "The username and password MUST NOT be the same.\n"
              "If the file does not exist, create it with owner-readable-only file permissions.\n"
              "It is also recommended to set alertnotify so you are notified of problems;\n"
              "for example: alertnotify=echo %%s | mail -s \"bzz Alert\" admin@foo.com\n"),
...
json_spirit::Value CRPCTable::execute(const std::string &strMethod, const json_spirit::Array &params, const Value& req_id) const
{
    ...
    if (!pcmd)
    {
        ...
        {
            throw JSONRPCError(RPC_NOT_SUPPORTED, "Method not available in cold version of _ebzz_ebzz");            
        }
    }
#ifdef ENABLE_WALLET
    if (pcmd->reqWallet && !pwalletMain)                                        // Never happens, reqWallet is changed to require Wallet read lock 
...
//    if(mc_gState->m_ProtocolVersionToUpgrade > mc_gState->m_NetworkParams->ProtocolVersion())
    if( (mc_gState->m_ProtocolVersionToUpgrade > 0) && (mc_gState->IsSupported(mc_gState->m_ProtocolVersionToUpgrade) == 0) )
    {
        ...
        {
            throw JSONRPCError(RPC_UPGRADE_REQUIRED, strprintf("BlockChain was upgraded to protocol version %d, please upgrade _ebzz_ebzz",mc_gState->m_ProtocolVersionToUpgrade));
        }
    }
...
std::string HelpExampleCli(string methodname, string args){
    return "> Instance request Examples->> " + std::string(mc_gState->m_NetworkParams->Name()) + " " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(string methodname, string args){
    return "> curl --user bzz_user --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
