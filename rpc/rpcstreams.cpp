...
Value publishmultifrom(const Array& params, bool fHelp)
{
    ...
    out_params.push_back(C_ebzz_ebzzAddress(out_address).ToString());
...
Value subscribe(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "API is not supported with this wallet version. To get this functionality, run \"bzzd -walletdbversion=2 -rescan\" ");        
    }   
...
Value liststreamtxitems(const Array& params, bool fHelp)
{
    ...
    if((mc_gState->m_WalletMode & MC_WMD_TXS) == 0)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "API is not supported with this wallet version. For full streams functionality, run \"bzzd -walletdbversion=2 -rescan\" ");        
    }   
