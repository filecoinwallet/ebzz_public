 ...
 */ 
    ...
    {
//        if(p_ebzz_ebzzFilterEngine->m_TxID != 0)
...
/* MCHN END */
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(C_ebzz_ebzzAddress(addr).ToString());
...
/* MCHN START */    
// TODO too many duplicate code with ListWalletTransactions and may be Accep_ebzz_ebzzTransaction
...
int VerifyNewTxForStreamFilters(const CTransaction& tx,std::string &strResult,mc__ebzz_ebzzFilter **lppFilter,int *applied)            
{
    ...
    if(p_ebzz_ebzzFilterEngine->NoStreamFilters())
    {
        return MC_ERR_NOERROR;
    }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) 
    {
        ...
        if(result.type() == obj_type)
        {
            ...
            int err=p_ebzz_ebzzFilterEngine->RunStreamFilters(tx,i,(unsigned char*)&hash+MC_AST_SHORT_TXID_OFFSET,-1,0, 
...
//        throw runtime_error("Help message not found\n");
    ...
    if(p_ebzz_ebzzFilterEngine->m_Vout < 0)
    {
    ...
    if(p_ebzz_ebzzFilterEngine->m_Vout >= (int)p_ebzz_ebzzFilterEngine->m_Tx.vout.size())
    {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "vout out of range");                                            
    }
    Value result=DataItemEntry(p_ebzz_ebzzFilterEngine->m_Tx,p_ebzz_ebzzFilterEngine->m_Vout,streams_already_seen, 0x0E00);
...
//    result.get_obj().push_back(Pair("txid", p_ebzz_ebzzFilterEngine->m_Tx.GetHash().GetHex()));
    result.get_obj().push_back(Pair("vout", p_ebzz_ebzzFilterEngine->m_Vout));
...
Value getfilterstream(const Array& params, bool fHelp)
{
    ...
    Value result=StreamEntry((unsigned char*)&(p_ebzz_ebzzFilterEngine->m_EntityTxID),0x0002);
...
//    if(p_ebzz_ebzzFilterEngine->m_TxID != 0)
    if(p_ebzz_ebzzFilterEngine->InFilter)
    {
        ...
        tx=p_ebzz_ebzzFilterEngine->m_Tx;
    }
    else
    {
        if(params.size() == 0)
...
*/    
    Object result;
    TxToJSON(p_ebzz_ebzzFilterEngine->m_Tx, 0, result);
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        BOOST_FOREACH(Value& input, inputs) {
            C_ebzz_ebzzAddress address(input.get_str());
            if (!address.IsValid())
...
/* MCHN END */       
        ...
        CTxDestination address;
        if (ExtractDestination(txout.scriptPubKey, address)) {
            entry.push_back(Pair("address", C_ebzz_ebzzAddress(address).ToString()));
...
/* MCHN START */                
                                                                                // _ebzz_ebzz issue #6056 with booust 1.58 fixed in https://github.com/_ebzz_ebzz/_ebzz_ebzz/commit/8b08d9530b93c7a98e7387167ecd2cd5b0563bfb
...
Value appendrawchange(const Array& params, bool fHelp)
{
    ...
    {
        if(Parse_ebzz_ebzzTxOutToBuffer(0,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
    ...
    {
        if(Parse_ebzz_ebzzTxOutToBuffer(tx.vin[i].prevout.hash,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
            nAmount+=txout.nValue;
        }
        i++;
...
Value disablerawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "disablerawtransaction supported only for protocol=_ebzz_ebzz");
    }    
...
Value decodescript(const Array& params, bool fHelp)
{
    ...
    r.push_back(Pair("p2sh", C_ebzz_ebzzAddress(CScriptID(script)).ToString()));
    return r;
}
