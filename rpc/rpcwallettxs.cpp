    ...
    BOOST_FOREACH(const CTxIn& txin, wtx.vin)
    {
        ...
        if (prevout_found)
        {
            if(IsTxOutMineAndInList(pwalletMain,prevout,addresses,filter))
            {
                ...
                if(Parse_ebzz_ebzzTxOutToBuffer(txin.prevout.hash,prevout,amounts,lpScript,NULL,NULL,strFailReason))
                {
    ...
    for (int i = 0; i < (int)wtx.vout.size(); ++i)
    {
        ...
        if(!txout.scriptPubKey.IsUnspendable())
        {
            ...
            if(IsTxOutMineAndInList(pwalletMain,txout,addresses,filter))
            {
                fIsMine=true;
                nIsToMeCount++;
                Parse_ebzz_ebzzTxOutToBuffer(wtx.GetHash(),txout,amounts,lpScript,NULL,&required,strFailReason);
...
//            Parse_ebzz_ebzzTxOutToBuffer(wtx.GetHash(),txout,amounts,lpScript,NULL,&required,strFailReason);
    ...
    if(total)
    {
        ...
        {
            aIssueAddresses.push_back(C_ebzz_ebzzAddress(addr).ToString());
        }
    ...
    {
        aMyAddresses.push_back(C_ebzz_ebzzAddress(addr).ToString());
    }
    
    BOOST_FOREACH(const CTxDestination& addr, from_addresses)
    {
        aFromAddresses.push_back(C_ebzz_ebzzAddress(addr).ToString());
    }
    
    BOOST_FOREACH(const CTxDestination& addr, to_addresses)
    {
//        aToAddresses.push_back(C_ebzz_ebzzAddress(addr).ToString());
