...
Value mcd_DebugIssueLicenseToken(const Object& params)
{
    ...
    C_ebzz_ebzzAddress from_address(mcd_ParamStringValue(params,"from_address",""));
    if (!from_address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    C_ebzz_ebzzAddress to_address(mcd_ParamStringValue(params,"to_address",""));
    if (!to_address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    C_ebzz_ebzzAddress req_address(mcd_ParamStringValue(params,"req_address",""));
...
//        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid request type");                    
    ...
    else
    {
        switch(request_type)
        {
            case MC_RMT_MC_ADDRESS_QUERY:
                ...
                C_ebzz_ebzzAddress address(addr_to_find);
                CKeyID keyID;
    ...
    if(request_type & MC_RMT_SPECIAL_MASK)
    {
        ...
        for(int a=0;a<attempts;a++)
        {
            ...
            remaining=_ebzz_ebzzCollectChunks(&collector);
            if(remaining == 0)
            {
                return 0;
            }
            ...
            if(params.size() > 3)
            {
                ...
                BOOST_FOREACH(CNode* pnode, vNodes) 
                {
                    ...
                    if( (params[3].get_str() == stats.addrName) || 
                        (params[3].get_str() == C_ebzz_ebzzAddress(stats.kAddrRemote).ToString()) )
                    {
                        pto=pnode;
                    }
                }
                    ...
                    case MC_RMT_MC_ADDRESS_QUERY:
                        ...
                        for(int r=0;r<(int)request->m_Responses.size();r++)
                        {
                            if(!res_found)
                            {
                                ...
                                {
                                    Array addresses;
                                    res.push_back(Pair("handshake",C_ebzz_ebzzAddress(node_addr.m_Address).ToString()));
