...
/* MCHN START */
bool Parse_ebzz_ebzzTxOutToBuffer(uint256 hash,const CTxOut& txout,mc_Buffer *amounts,mc_Script *lpScript,int *allowed,int *required,string& strFailReason);
...
/* MCHN START */    
    ...
    if(mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz())
    {
        ...
        {
            miner=C_ebzz_ebzzAddress(keyID).ToString();
        }
    }
...
//        throw JSONRPCError(RPC_INVALID_PARAMS, "Wrong number of parameters");                    
    ...
    if( (vin < 0) || (vin >= (unsigned int)p_ebzz_ebzzFilterEngine->m_Tx.vin.size()) )
    {
    ...
    Array getxoutparams;
    getxoutparams.push_back(p_ebzz_ebzzFilterEngine->m_Tx.vin[vin].prevout.hash.ToString());
    getxoutparams.push_back((int64_t)p_ebzz_ebzzFilterEngine->m_Tx.vin[vin].prevout.n);
...
Value gettxout(const Array& params, bool fHelp)
{
    ...
    {
        if (!((CCoinsViewCache*)(p_ebzz_ebzzFilterEngine->m_CoinsCache))->GetCoins(hash, coins))
            return Value::null;        
    }
    else
    {
        if (fMempool) {
            ...
            if (!view.GetCoins(hash, coins))
                return Value::null;
            if(p_ebzz_ebzzFilterEngine->InFilter() == 0)                                // In filter we already checked this input exists, but mempool is dirty
            {
