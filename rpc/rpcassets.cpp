...
Value getmultibalances(const Array& params, bool fHelp)
{
    ...
    if(params.size() > 0)
    {
        ...
        {        
            ...
            {
                C_ebzz_ebzzAddress address(str_addr);
                dest=address.Get();
    ...
    
    {
        ...
        {        
            ...
            str_addr=C_ebzz_ebzzAddress(address).ToString();
    ...
    if (params.size() > 0)
    {
        if (params[0].get_str() != "*") 
        {
            if (params[0].get_str() != "") 
            {
                ...
                {
                    throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need getassetbalances, run bzz -walletdbversion=1 -rescan, but the wallet will perform worse");        
                }            
            }
            BOOST_FOREACH(const PAIRTYPE(C_ebzz_ebzzAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
            {
...
//                        aMetaData.push_back(OpReturnEntry(elem,elem_size,wtx.GetHash(),i));
    ...
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& item, mAddresses)
    {
        ...
        {
            oBalance.push_back(Pair(C_ebzz_ebzzAddress(item.first).ToString(), units*item.second));            
        }
    }
    if(other_amount != 0)
    {
...
Value getfilterassetbalances(const Array& params, bool fHelp)
{
    ...
    for(int j=0;j<(int)p_ebzz_ebzzFilterEngine->m_Tx.vin.size();j++)
    {
        int n=p_ebzz_ebzzFilterEngine->m_Tx.vin[j].prevout.n;
        CCoins coins;
        {
            if(p_ebzz_ebzzFilterEngine->m_CoinsCache)
            {
                if (!((CCoinsViewCache*)(p_ebzz_ebzzFilterEngine->m_CoinsCache))->GetCoins(p_ebzz_ebzzFilterEngine->m_Tx.vin[j].prevout.hash, coins))
                    return Value::null;        
            }
            else
            {
                ...
                if (!view.GetCoins(p_ebzz_ebzzFilterEngine->m_Tx.vin[j].prevout.hash, coins))
                {
                    return Value::null;                                                                 
                }
            }
                ...
                if(CreateAssetBalanceList(coins.vout[n],asset_amounts,lpScript,&required))
                {
                    for(int i=0;i<asset_amounts->GetCount();i++)
                    {
                        ...
                        else
                        {
                            ...
                            if(memcmp(lpAsset->GetTxID(),&(p_ebzz_ebzzFilterEngine->m_Tx.vin[gin].prevout.hash),sizeof(uint256)) == 0)
                            {
    ...
    for (int j = 0; j < (int)p_ebzz_ebzzFilterEngine->m_Tx.vout.size(); ++j)
    {
        const CTxOut& txout = p_ebzz_ebzzFilterEngine->m_Tx.vout[j];
        if(!txout.scriptPubKey.IsUnspendable())
        {
            CTxDestination address;
            int required=0;
            ...
            if(lpAsset)
            {
                ...
                if(CreateAssetBalanceList(txout,asset_amounts,lpScript,&required))
                {
                    for(int i=0;i<asset_amounts->GetCount();i++)
                    {
                        ...
                        {
                            if(memcmp(lpAsset->GetTxID(),&(p_ebzz_ebzzFilterEngine->m_TxID),sizeof(uint256)) == 0)
                            {
                                if( mc_GetABRefType(asset_amounts->GetRow(i)) == MC_AST_ASSET_REF_TYPE_GENESIS )                    
                                {
                                    quantity=mc_GetABQuantity(asset_amounts->GetRow(i));
    ...
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64_t)& item, mAddresses)
    {
        ...
        {
            oBalance.push_back(Pair(C_ebzz_ebzzAddress(item.first).ToString(), raw_value ? item.second : item.second/multiple));            
        }
    }
    if(other_amount != 0)
    {
...
Value getassettransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "API is not supported with this wallet version. To get this functionality, run \"bzz -walletdbversion=2 -rescan\" ");        
    }   
