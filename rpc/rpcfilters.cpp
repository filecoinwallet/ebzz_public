...
/*
bool Accept_ebzz_ebzzTransaction(const CTransaction& tx, 
...
*/    
    ...
    err=pFilterEngine->CreateFilter(test_code.c_str(),filter_main_name,p_ebzz_ebzzFilterEngine->m_CallbackNames[filter_type],worker,strError);
    delete worker;
    if(err)
    {
...
Value listfilters(const Array& params, uint32_t filter_type)
{
    ...
    for(int i=0;i<(int)p_ebzz_ebzzFilterEngine->m_Filters.size();i++)
    {
        ...
        if((filter_list.size() == 0) || 
           (filter_list.find(*(uint256*)p_ebzz_ebzzFilterEngine->m_Filters[i].m_Details.GetTxID()) != filter_list.end()) )
        {
            entry=FilterEntry(p_ebzz_ebzzFilterEngine->m_Filters[i].m_Details.GetTxID(),output_level,filter_type);
            if(entry.size()>0)
            {
                bool take_it=false;
                ...
                if(take_it)
                {
                    bool valid=p_ebzz_ebzzFilterEngine->m_Filters[i].m_CreateError.size() == 0;
                    entry.push_back(Pair("compiled",valid));
                    if(check_approval)
                    {
                        entry.push_back(Pair("approved",mc_gState->m_Permissions->FilterApproved(lpEntity,&(p_ebzz_ebzzFilterEngine->m_Filters[i].m_FilterAddress)) !=0 ));
                        entry.push_back(Pair("address",p_ebzz_ebzzFilterEngine->m_Filters[i].m_FilterAddress.ToString()));
                    }
...
Value getfiltertxid(const Array& params, bool fHelp)
{
    ...
    return p_ebzz_ebzzFilterEngine->m_TxID.ToString();
}
...
Value setfilterparam(const json_spirit::Array& params, bool fHelp)
{
    ...
    if( (param_name == "maxshowndata") )
    {
        if( (params[1].type() == int_type) || (params[1].type() == str_type) )
        {
            ...
            p_ebzz_ebzzFilterEngine->m_Params.m_MaxShownData=nValue;
        }
        else
        {
...
Value testfilter(const vector <uint160>& entities,string filter_code, Value txparam,int vout_in,uint32_t filter_type,string library_code)
{
    ...
    mc_Filter *worker=new mc_Filter;
//    err=pFilterEngine->CreateFilter(filter_code,filter_main_name,p_ebzz_ebzzFilterEngine->m_CallbackNames[filter_type],worker,strError);
    ...
    err=pFilterEngine->CreateFilter(test_code.c_str(),filter_main_name,p_ebzz_ebzzFilterEngine->m_CallbackNames[filter_type],worker,p_ebzz_ebzzFilterEngine->GetSendTimeout(), strError);
    if(err)
    {
        ...
        if(relevant_filter)
        {
            ...
            if ( (filter_type == MC_FLT_TYPE_TX) && !txid_given)
            {
...

                {
                    ...
                    if(!Accept_ebzz_ebzzTransaction(tx,view,-1,MC_AMT_NO_FILTERS,reason,NULL,NULL))
                    {
            ...
            err=p_ebzz_ebzzFilterEngine->RunFilterWithCallbackLog(tx,vout,stream_txid,worker,strError,callbacks);
            if(err)
            {
                errorCode=RPC_INTERNAL_ERROR;
