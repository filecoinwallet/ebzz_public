...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 0)
    {
        ...
        {
            throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need accounts, run bzz -walletdbversion=1 -rescan, but the wallet will perform worse");        
        }            
    ...
    return C_ebzz_ebzzAddress(keyID).ToString();
}


C_ebzz_ebzzAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return C_ebzz_ebzzAddress(account.vchPubKey.GetID());
}
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    C_ebzz_ebzzAddress address(params[0].get_str());
...
Value listaddresses(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "API is not supported with this wallet version. To get this functionality, run \"_ebzz_ebzzd -walletdbversion=2 -rescan\" ");        
    }   
    ...
    for(int i=0;i<entity_count;i++)
    {
            ...
            ((lpEntity->m_Flags & MC_EFL_NOT_IN_LISTS) == 0 ) )               
        {
            if(setAddresses.size())
            {
                ...
                if(C_ebzz_ebzzAddressFromTxEntity(address,&(lpEntity->m_Entity)))
                {
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(C_ebzz_ebzzAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(C_ebzz_ebzzAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(C_ebzz_ebzzAddress(address).Get())->second.name);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    ...
    CKey key;
    C_ebzz_ebzzAddress addr(strAddress);
    if (!addr.IsValid())
    {
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Not supported with scalable wallet - if you need accounts, run bzz -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }            
    
    // _ebzz_ebzz address
    C_ebzz_ebzzAddress address = C_ebzz_ebzzAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
Value getbalance(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need getbalance, run bzz -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
    string strAccount = AccountFromValue(params[0]);
...
Value movecmd(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need move, run bzz -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
...
Value sendfrom(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need sendfrom, run bzz -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
    
    string strAccount = AccountFromValue(params[0]);
    C_ebzz_ebzzAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    CAmount nAmount = AmountFromValue(params[2]);
    int nMinDepth = 1;
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need sendmany, run bzz -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
    ...
    {
        C_ebzz_ebzzAddress address(s.name_);
        if (!address.IsValid())
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    return C_ebzz_ebzzAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<C_ebzz_ebzzAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(C_ebzz_ebzzAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<C_ebzz_ebzzAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value listtransactions(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Not supported with scalable wallet - if you need listtransactions, run bzz -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
    else
    {
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need listaccounts, run bzz -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }   
