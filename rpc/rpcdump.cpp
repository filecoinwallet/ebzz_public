...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    bool fNewFound=false;
    vector<string> inputStrings=ParseStringList(params[0]);
    vector<C_ebzz_ebzzAddress> inputAddresses;
    ...
    for(int is=0;is<(int)inputStrings.size();is++)
    {
        ...
        C_ebzz_ebzzAddress address(param);
        if (address.IsValid()) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid bzz address or script");
        }
    ...
    for(int is=0;is<(int)inputStrings.size();is++)
    {
        C_ebzz_ebzzAddress address=inputAddresses[is];
        ...
        else
        {
            ...
            address=C_ebzz_ebzzAddress(innerID);
        }
...
Value importwallet(const Array& params, bool fHelp)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", C_ebzz_ebzzAddress(keyid).ToString());
//                continue;
            fAlreadyHave=true;
        }
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    ...
    return C_ebzz_ebzzSecret(vchSecret).ToString();
}
...
Value dumpwallet(const Array& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by bzz %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = C_ebzz_ebzzAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            if (pwalletMain->mapAddressBook.count(keyid)) {
                file << strprintf("%s %s label=%s # addr=%s\n", C_ebzz_ebzzSecret(key).ToString(), strTime, EncodeDumpString(pwalletMain->mapAddressBook[keyid].name), strAddr);
            } else if (setKeyPool.count(keyid)) {
                file << strprintf("%s %s reserve=1 # addr=%s\n", C_ebzz_ebzzSecret(key).ToString(), strTime, strAddr);
            } else {
