...
using namespace std;
void Push_ebzz_ebzzRelay(CNode* pto, uint32_t msg_type,vector<CAddress>& path,vector<CAddress>& path_to_follow,vector<unsigned char>& payload);
...
/* MCHN START */        
        if(mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz())
        {
            ...
            {
                obj.push_back(Pair("handshakelocal", C_ebzz_ebzzAddress(stats.kAddrLocal).ToString()));                
                obj.push_back(Pair("handshake", C_ebzz_ebzzAddress(stats.kAddrRemote).ToString()));                
            }
        }
/* MCHN END */        
...
//                    Push_ebzz_ebzzRelay(pnode, MC_RMT_GLOBAL_PING,path,path_to_follow,payload);
...
/* MCHN START */    
    if(mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz())
    {
        obj.push_back(Pair("subversion",
            FormatSubVersion("_ebzz_ebzz", mc_gState->GetProtocolVersion(), std::vector<string>())));
    }
    else
    {
        obj.push_back(Pair("subversion",
