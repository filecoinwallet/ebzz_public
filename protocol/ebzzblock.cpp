                            ...
                            if(ptr)
                            {
                                ...
                                while(ptr<ptrEnd)
                                {
                                    ...
                                    if(param.m_Param)
                                    {
                                        ...
                                        if(vUpgrades == NULL)                   // Called from _ebzz_ebzzNode_ApplyUpgrades
                                        {
        ...
        if(pool.exists(hash))
        {
            ...
            else
            {
                ...
                if(entry.FullReplayRequired())
                {
                    ...
                    if(!Accept_ebzz_ebzzTransaction(tx,view,-1,accept ? MC_AMT_DEFAULT : MC_AMT_NO_ACCEPT,reason,NULL,NULL))
                    {
    ...
    for(pos=0;pos<branch_size;pos++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if(mc_gState->m_Permissions->CanMineBlockOnFork(&miners[pos],pindex->nHeight,last_after_fork) == 0)
                {
                    LogPrintf("VerifyBlockMiner: Permission denied for miner %s received in block signature\n",C_ebzz_ebzzAddress(pubKeyHashNew).ToString().c_str());
                    fReject=true;
                    goto exitlbl;                    
                }
            }
