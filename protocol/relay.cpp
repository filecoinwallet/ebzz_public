...
uint32_t _ebzz_ebzzNextChunkQueryAttempt(uint32_t attempts)
{
...
bool _ebzz_ebzzProcessChunkResponse(const CRelayResponsePair *response_pair,map <int,int>* request_pairs,mc_ChunkCollector* collector)
{
...
int _ebzz_ebzzResponseScore(mc_RelayResponse *response,mc_ChunkCollectorRow *collect_row,map<int64_t,int64_t>& destination_loads,uint32_t max_total_size)
{
    unsigned char *ptr;
    unsigned char *ptrEnd;
    unsigned char *ptrStart;
...
int _ebzz_ebzzCollectChunks(mc_ChunkCollector* collector)
{
...
//                    printf("coll new rsp: row: %d, id: %s, %d\n",row,collect_row->m_State.m_Request.ToString().c_str(),collect_row->m_State.m_RequestPos);
    ...
    {
        _ebzz_ebzzProcessChunkResponse(&(item.first),&(item.second.m_Pairs),collector);
            ...
            if(collect_row->m_State.m_Request.IsZero())
            {
                ...
                if(!collect_row->m_State.m_Query.IsZero())
                {
                    ...
                    if(query == NULL)
                    {
                        ...
                        collect_row->m_State.m_QueryNextAttempt=time_now+_ebzz_ebzzNextChunkQueryAttempt(collect_row->m_State.m_QueryAttempts);                                                
                ...
                if(query)
                {
                    ...
                    {
                        this_score=_ebzz_ebzzResponseScore(&(query->m_Responses[i]),collect_row,destination_loads,max_total_destination_size);
...
int _ebzz_ebzzCollectChunksQueueStats(mc_ChunkCollector* collector)
{
    int row;
    int delay;
    mc_ChunkCollectorRow *collect_row;
...
bool _ebzz_ebzzRelayResponse(uint32_t msg_type_stored, CNode *pto_stored,
...
//    printf("recv: %d, from: %d, to: %d, hc: %d, size: %d, ts: %u, nc: %u\n",msg_type_in,pfrom->GetId(),pto_stored ? pto_stored->GetId() : 0,hop_count,(int)vPayloadIn.size(),timestamp_received,nonce_received);
    ...
    else
    {
        ...
        {
            if(_ebzz_ebzzRelayResponse(msg_type_stored,pto_stored,
