...
// Copyright (c) 2009-2016 The _ebzz_ebzz developers
...
#include "filters/_ebzz_ebzzfilter.h"
...
typedef struct C_ebzz_ebzzTxDetails
{    
...
void C_ebzz_ebzzTxDetails::Zero()
{
...
bool C_ebzz_ebzzTxDetails::IsRelevantInput(int vin, int vout)
{
...
bool C_ebzz_ebzzTxDetails::IsRelevantEntity(uint160 hash)
{
...
void C_ebzz_ebzzTxDetails::SetRelevantEntity(void *entity)
{
    uint160 hash=0;
...
void C_ebzz_ebzzTxDetails::SetAffectedLibrary(void *entity)
{
    uint160 hash=0;
    memcpy(&hash,entity,MC_AST_SHORT_TXID_SIZE);
...
bool mc_CompareAssetQuantities(C_ebzz_ebzzTxDetails *details,string& reason)
{
    unsigned char *ptrIn;
...
void _ebzz_ebzzTransaction_SetTmpOutputScript(const CScript& script1)
{
    CScript::const_iterator pc1 = script1.begin();
    mc_gState->m_TmpScript->Clear();
    mc_gState->m_TmpScript->SetScript((unsigned char*)(&pc1[0]),(size_t)(script1.end()-pc1),MC_SCR_TYPE_SCRIPTPUBKEY);
...
bool _ebzz_ebzzTransaction_CheckCachedScriptFlag(const CTransaction& tx)
{
    bool flag=false;
...
bool _ebzz_ebzzTransaction_CheckCoinbaseInputs(const CTransaction& tx,     
                                               C_ebzz_ebzzTxDetails *details)
{
    if(!tx.IsCoinBase())                                                  
    {   
        return false;
...
bool _ebzz_ebzzTransaction_CheckInputs(const CTransaction& tx,                  // Tx to check
                                       const CCoinsViewCache &inputs,           // Tx inputs from UTXO database
                                       C_ebzz_ebzzTxDetails *details,           // Tx details object
...
bool _ebzz_ebzzTransaction_CheckNewEntity(int vout,
                                          bool& fScriptParsed,
                                          C_ebzz_ebzzTxDetails *details,   
                                          string& reason)      
{
...
bool _ebzz_ebzzTransaction_CheckCachedScript(const CTransaction& tx, 
...
void _ebzz_ebzzTransaction_FillAdminPermissionsBeforeTx(const CTransaction& tx,
                                                        C_ebzz_ebzzTxDetails *details)      
{
...
bool _ebzz_ebzzTransaction_VerifyAndDeleteDataFormatElements(string& reason,int64_t *total_size,uint32_t *salt_size)
{    
...
bool _ebzz_ebzzTransaction_CheckOpReturnScript(const CTransaction& tx, 
                                               const CCoinsViewCache &inputs,   
                                               int vout,
                                               C_ebzz_ebzzTxDetails *details,   
                                               string& reason)      
{
    ...
    total_offchain_size=0;
    if(!_ebzz_ebzzTransaction_VerifyAndDeleteDataFormatElements(reason,&total_offchain_size,NULL))
    {
        return false;
    }
    ...
    if( mc_gState->m_TmpScript->GetNumElements() == 2 )                         // Cached input script or new entity
    {
        ...
        {
                                                                                // Cached scripts
            if(!_ebzz_ebzzTransaction_CheckCachedScript(tx, inputs, fScriptParsed, details, reason))
            {
                return false;
            }
        }
        ...
        {
                                                                                // New entities
            if(!_ebzz_ebzzTransaction_CheckNewEntity(vout, fScriptParsed, details, reason))
            {
                return false;
            }
        }
                                                                                ...
                                                                                // Performed only if it is indeed needed
    {
        ...
        {
            _ebzz_ebzzTransaction_FillAdminPermissionsBeforeTx(tx,details);
        }                        
    }
...
bool _ebzz_ebzzTransaction_CheckAssetUpdateDetails(mc_EntityDetails *entity,
                                                   int vout,
                                                   C_ebzz_ebzzTxDetails *details,   
                                                   string& reason)      
{
...
bool _ebzz_ebzzTransaction_CheckVariableUpdateDetails(const CTransaction& tx,
                                                   mc_EntityDetails *entity,
                                                   int offset, 
                                                   int vout,
                                                   C_ebzz_ebzzTxDetails *details,   
...
bool _ebzz_ebzzTransaction_CheckUpgradeApproval(const CTransaction& tx,
                                                mc_EntityDetails *entity,
                                                int offset,  
                                                int vout,
                                                C_ebzz_ebzzTxDetails *details,   
...
bool _ebzz_ebzzTransaction_CheckStreamItem(mc_EntityDetails *entity,
                                           int vout,
                                           C_ebzz_ebzzTxDetails *details,   
                                           string& reason)      
{
...
bool _ebzz_ebzzTransaction_CheckEntityItem(const CTransaction& tx,
                                           int offset,  
                                           int vout,
                                           C_ebzz_ebzzTxDetails *details,   
                                           string& reason)      
{
    ...
    if(!_ebzz_ebzzTransaction_VerifyAndDeleteDataFormatElements(reason,NULL,&salt_size))
    {
        return false;
    }
       ...
       (entity.GetEntityType() == MC_ENT_TYPE_LIBRARY))                         // Asset, variable or library update
    {
        if(entity.GetEntityType() == MC_ENT_TYPE_ASSET)
        {
            if(!_ebzz_ebzzTransaction_CheckAssetUpdateDetails(&entity,vout,details,reason))
            {
                return false;            
            }
        }
        ...
        {
            if(!_ebzz_ebzzTransaction_CheckVariableUpdateDetails(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
    ...
    else
    {
        ...
        {
            if(!_ebzz_ebzzTransaction_CheckUpgradeApproval(tx,&entity,offset,vout,details,reason))
            {
                return false;            
            }
        }
        else                                                                    // (Pseudo)stream item
        {
            if(entity.GetEntityType() <= MC_ENT_TYPE_STREAM_MAX)
            {
                ...
                if(!_ebzz_ebzzTransaction_CheckStreamItem(&entity,vout,details,reason))
                {
                    return false;            
                }            
            }
...
bool _ebzz_ebzzTransaction_CheckDestinations(const CScript& script1,
                                             int vout,
                                             C_ebzz_ebzzTxDetails *details,   
                                             string& reason)      
{
...
bool _ebzz_ebzzTransaction_CheckIfLibraryApproval(unsigned char* ptr,
                                                  C_ebzz_ebzzTxDetails *details)
{
    mc_EntityDetails entity;
...
bool _ebzz_ebzzTransaction_ProcessPermissions(const CTransaction& tx,
                                              int offset,  
                                              int vout,
        ...
        {   
            if(mc_gState->m_TmpScript->GetPermission(&type,&from,&to,&timestamp) == 0) // Grant script
            {
                ...
                if(type)
                {
                    ...
                    C_ebzz_ebzzAddress address;
                    unsigned char* ptr=NULL;
                    flags=MC_PFL_NONE;
                    if(lpKeyID != NULL)
                    {
                        address=C_ebzz_ebzzAddress(*lpKeyID);
                    ...
                    {
                        flags=MC_PFL_IS_SCRIPTHASH;
                        address=C_ebzz_ebzzAddress(*lpScriptID);
                            ...
                            if(details->vInputDestinations[i] != 0)
                            {
                                if( ( (type & (MC_PTP_ADMIN | MC_PTP_MINE)) == 0) || details->vInputCanGrantAdminMine[i] || (entity.GetEntityType() > 0) )
                                {
                                            ...
                                            from,to,timestamp,flags,1,offset) == 0)
                                    {
                                        if(type & MC_PTP_FILTER)
                                        {
                                            _ebzz_ebzzTransaction_CheckIfLibraryApproval(ptr,details);
                                        }
                                        fAdminFound=true;
                                    }
                                }
...
bool Is__ebzz_ebzzLicenseTokenTransfer(const CTransaction& tx)
{
    mc_EntityDetails entity;
    if(tx.IsCoinBase())
    {
    ...
    _ebzz_ebzzTransaction_SetTmpOutputScript(tx.vout[0].scriptPubKey);
...
bool _ebzz_ebzzTransaction_CheckLicenseTokenTransfer(const CTransaction& tx,
                                               int unchecked_row, 
                                               C_ebzz_ebzzTxDetails *details,   
                                               string& reason)      
{
...
bool _ebzz_ebzzTransaction_CheckAssetTransfers(const CTransaction& tx,
                                               int offset,  
                                               int vout,
                                               C_ebzz_ebzzTxDetails *details,   
                                               string& reason)      
{
    ...
    if(!_ebzz_ebzzTransaction_CheckLicenseTokenTransfer(tx,unchecked_row,details,reason))
    {
        return false;
    }
...
bool _ebzz_ebzzTransaction_CheckOutputs(const CTransaction& tx,                 // Tx to check
                                        const CCoinsViewCache &inputs,          // Tx inputs from UTXO database
                                        ...
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // Basic checks, destinations and simple grants
    {
        ...
        _ebzz_ebzzTransaction_SetTmpOutputScript(tx.vout[vout].scriptPubKey);

        if(mc_gState->m_TmpScript->IsOpReturnScript())                    
        {
            if(!_ebzz_ebzzTransaction_CheckOpReturnScript(tx,inputs,vout,details,reason))
            {
                return false;
            }
        }
        ...
        {
            if(!_ebzz_ebzzTransaction_CheckDestinations(tx.vout[vout].scriptPubKey,vout,details,reason))
            {
                return false;
            }            
            ...
            if(!_ebzz_ebzzTransaction_ProcessPermissions(tx,offset,vout,permission_type,true,details,reason))
            {
                return false;
            }            
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)                  // create, issue, activate grants (requiring pre-tx permissions)
    {
        if(details->vOutputScriptFlags[vout] & (MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_CREATE | MC_MTX_OUTPUT_DETAIL_FLAG_PERMISSION_FILTER) )
        {
            ...
            if(!_ebzz_ebzzTransaction_ProcessPermissions(tx,offset,vout,permission_type,false,details,reason))
            {
                return false;
            }                        
        }
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_OP_RETURN_ENTITY_ITEM)
        {
            ...
            if(!_ebzz_ebzzTransaction_CheckEntityItem(tx,offset,vout,details,reason))
            {
                return false;                
            }
...
bool _ebzz_ebzzTransaction_CheckTransfers(const CTransaction& tx,               // Tx to check
                                        int offset,                             // Tx offset in block, -1 if in memppol
                                        C_ebzz_ebzzTxDetails *details,          // Tx details object
                                        string& reason)                         // Error message
{
    ...
    for (unsigned int vout = 0; vout < tx.vout.size(); vout++)
    {
        ...
        if(details->vOutputScriptFlags[vout] & MC_MTX_OUTPUT_DETAIL_FLAG_NOT_OP_RETURN)
        {
            ...
            if(!_ebzz_ebzzTransaction_CheckAssetTransfers(tx,offset,vout,details,reason))
            {
                return false;                
            }
        }        
...
int64_t _ebzz_ebzzTransaction_OffchainFee(int64_t total_offchain_size)            // Total size of offchain items
{
    return (MIN_OFFCHAIN_FEE*total_offchain_size + 999)/ 1000;
}

bool _ebzz_ebzzTransaction_CheckMandatoryFee(C_ebzz_ebzzTxDetails *details,     // Tx details object
                                             int64_t *mandatory_fee,            // Mandatory Fee    
                                             string& reason)                    // Error message
{
    *mandatory_fee = _ebzz_ebzzTransaction_OffchainFee(details->total_offchain_size);
...
bool _ebzz_ebzzTransaction_CheckLicenseTokenDetails(C_ebzz_ebzzTxDetails *details,     // Tx details object
...
bool _ebzz_ebzzTransaction_ProcessAssetIssuance(const CTransaction& tx,         // Tx to check
                                                int offset,                     // Tx offset in block, -1 if in memppol
            ...
            (details->new_entity_type != MC_ENT_TYPE_LIBRARY) )
        {
            ...
            {                    
                if(mc_gState->m_Features->LicenseTokens())
                {
                    ...
                    _ebzz_ebzzTransaction_SetTmpOutputScript(tx.vout[issue_vout].scriptPubKey);
                    if(mc_gState->m_Features->License20010())
                    {
                        if(mc_gState->m_TmpScript->GetNumElements() < 2)
                        {
                    ...
                    if(!_ebzz_ebzzTransaction_CheckLicenseTokenDetails(details,token_address,reason))
                    {
                        return false;                                                                                                                                                        
                    }
...
bool _ebzz_ebzzTransaction_ProcessEntityCreation(const CTransaction& tx,        // Tx to check
                                                 int offset,                    // Tx offset in block, -1 if in memppol
                                                 bool accept,                   // Accept to mempools if successful
                                                 C_ebzz_ebzzTxDetails *details, // Tx details object
                                                 string& reason)                // Error message
...
//                if( (details->new_entity_type <= MC_ENT_TYPE_STREAM_MAX) ||     // Admin persmission is required for upgrades and filters
    ...
    if(update_mempool)
    {
        if(mc_gState->m_Assets->FindEntityByTxID(&entity,(unsigned char*)&txid))
        {
            ...
            if(details->new_entity_type == MC_ENT_TYPE_FILTER)
            {
                p_ebzz_ebzzFilterEngine->AddFilter((unsigned char*)&txid+MC_AST_SHORT_TXID_OFFSET,(offset < 0) ? 0 : 1);
            }
        }
        else
        {
...
bool _ebzz_ebzzTransaction_VerifyNotFilteredRestrictions(const CTransaction& tx,        // Tx to check
                                                         int offset,                    // Tx offset in block, -1 if in memppol
                                                         bool accept,                   // Accept to mempools if successful
                                                         C_ebzz_ebzzTxDetails *details, // Tx details object
                                                         string& reason)                // Error message
...
bool _ebzz_ebzzTransaction_VerifyStandardCoinbase(const CTransaction& tx,        // Tx to check
                                                        C_ebzz_ebzzTxDetails *details, // Tx details object
                                                        string& reason)                // Error message
{
    ...
    if(signature_output == (mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz() == 0))
    {
        LogPrintf("Non-standard coinbase: Sigature output doesn't match protocol\n");
        return true;                                                            // Signature output should appear only if protocol=_ebzz_ebzz
    }
...
bool _ebzz_ebzzTransaction_ProcessLibraryUpdates(int offset,
                                                 C_ebzz_ebzzTxDetails *details, // Tx details object                                                 
                                                 string& reason)                // Error message
{
    ...
    if(p_ebzz_ebzzFilterEngine)
    {    
        p_ebzz_ebzzFilterEngine->CheckLibraries(&(details->vAffectedLibraries),(offset >= 0));
    }
    return true;
}


bool Accept_ebzz_ebzzTransaction   (const CTransaction& tx,                     // Tx to check
                                    const CCoinsViewCache &inputs,              // Tx inputs from UTXO database
                                    int offset,                                 // Tx offset in block, -1 if in memppol
                                    ...
                                    uint32_t *replay)                           // Replay flag - if tx should be rechecked or only permissions
{
    ...
    if(mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz() == 0)                 
    {
        return true;
    }    
    
    details.fCheckCachedScript=_ebzz_ebzzTransaction_CheckCachedScriptFlag(tx);
    
    mc_gState->m_TmpAssetsIn->Clear();
    
    if(!_ebzz_ebzzTransaction_CheckCoinbaseInputs(tx,&details))                 // Inputs
    {
        if(!_ebzz_ebzzTransaction_CheckInputs(tx,inputs,&details,reason))
        {
            return false;
        }
    }
    ...
    if(!_ebzz_ebzzTransaction_CheckOutputs(tx,inputs,offset,&details,reason))   // Outputs        
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
    
    if(!_ebzz_ebzzTransaction_ProcessAssetIssuance(tx,offset,accept,&details,reason))                // Asset genesis/followon
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!_ebzz_ebzzTransaction_CheckMandatoryFee(&details,&mandatory_fee,reason))
    {
        fReject=true;
        goto exitlbl;                                                                            
    }                                                                           

    if(!_ebzz_ebzzTransaction_CheckTransfers(tx,offset,&details,reason))        // Transfers and receive permissions
    {
        fReject=true;
        goto exitlbl;                                                                    
    }
                                                                                // Creating of (pseudo)streams/upgrades
    if(!_ebzz_ebzzTransaction_ProcessEntityCreation(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                    
    }

    if(!_ebzz_ebzzTransaction_VerifyNotFilteredRestrictions(tx,offset,accept,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!_ebzz_ebzzTransaction_VerifyStandardCoinbase(tx,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
    
    if(!_ebzz_ebzzTransaction_ProcessLibraryUpdates(offset,&details,reason))           
    {
        fReject=true;
        goto exitlbl;                                                                                
    }        
        ...
        ((flags & MC_AMT_NO_FILTERS) == 0) )
    {
        if(mc_gState->m_Features->Filters())
        {
            if(p_ebzz_ebzzFilterEngine)
            {
                mc__ebzz_ebzzFilter* lpFilter;
                int applied=0;

                if(p_ebzz_ebzzFilterEngine->RunTxFilters(tx,details.vRelevantEntities,reason,&lpFilter,&applied,(offset >= 0)) != MC_ERR_NOERROR)
                {
...
exitlbl:
    ...
    if(!accept || fReject)                                                      // Rolling back permission database if we were just checking or error occurred    
    {
        ...
        _ebzz_ebzzTransaction_ProcessLibraryUpdates(offset,&details,reason);
    }
