...
bool _ebzz_ebzzNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !_ebzz_ebzzNode_CanConnect(pnode);
    }
...
bool _ebzz_ebzzNode_AcceptData(CNode *pnode)
{
    ...
    return !_ebzz_ebzzNode_DisconnectRemote(pnode);
}
...
bool Verify_ebzz_ebzzVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
{   
    ...
    {
        if(!Verify_ebzz_ebzzVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//                return false;
    ...
    if(MCP_ANYONE_CAN_CONNECT == 0)
    {
        ...
        if(fIsVerackack)
        {
            LogPrintf("mchn: Connection from %s received on peer=%d in verackack (%s)\n",C_ebzz_ebzzAddress(pubKeyHash).ToString().c_str(), pfrom->id,pfrom->addr.ToString());
            if(!_ebzz_ebzzNode_CanConnect(pfrom))
            {
                LogPrintf("mchn: Permission denied for address %s received from peer=%d\n",C_ebzz_ebzzAddress(pubKeyHash).ToString().c_str(), pfrom->id);
...
//            if(pfrom->fDefaultMessageStart)
    ...
    {
        C_ebzz_ebzzAddress address(mapArgs["-handshakelocal"]);
    ...
    
    {
        ...
        if(pNodeStatus)
        {
            ...
            {
                pNodeStatus->sAddress=C_ebzz_ebzzAddress(pkey.GetID()).ToString();
            }
        }
    }
