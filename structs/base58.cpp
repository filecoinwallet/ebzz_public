...
namespace
{
class C_ebzz_ebzzAddressVisitor : public boost::static_visitor<bool>
{
private:
    C_ebzz_ebzzAddress* addr;

public:
    C_ebzz_ebzzAddressVisitor(C_ebzz_ebzzAddress* addrIn) : addr(addrIn) {}
...
/* MCHN START */
bool C_ebzz_ebzzAddress::Set(const CKeyID &id,const std::vector<unsigned char>& vchV)
{
    SetData(vchV, &id, 20);
    return true;    
}
...
bool C_ebzz_ebzzAddress::Set(const CKeyID& id)
{
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool C_ebzz_ebzzAddress::Set(const CScriptID& id)
{
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool C_ebzz_ebzzAddress::Set(const CTxDestination& dest)
{
    return boost::apply_visitor(C_ebzz_ebzzAddressVisitor(this), dest);
}

bool C_ebzz_ebzzAddress::IsValid() const
{
    return IsValid(Params());
}

bool C_ebzz_ebzzAddress::IsValid(const CChainParams& params) const
{
...
CTxDestination C_ebzz_ebzzAddress::Get() const
{
    if (!IsValid())
...
bool C_ebzz_ebzzAddress::GetKeyID(CKeyID& keyID) const
{
...
bool C_ebzz_ebzzAddress::GetScriptID(CScriptID& scriptID) const
{
...
bool C_ebzz_ebzzAddress::IsScript() const
{
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void C_ebzz_ebzzSecret::SetKey(const CKey& vchSecret)
{
...
bool C_ebzz_ebzzSecret::IsValid() const
{
...
bool C_ebzz_ebzzSecret::SetString(const char* pszSecret)
{
...
bool C_ebzz_ebzzSecret::SetString(const std::string& strSecret)
{
