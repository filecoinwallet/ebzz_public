...
bool AppInit(int argc, char* argv[])
{
    ...
    {
        fprintf(stderr,"\nError: Data directory %s needs to exist before calling bzz. Exiting...\n\n",mapArgs["-datadir"].c_str());
        delete mc_gState;
        return false;        
    }
    ...
    {
        fprintf(stderr,"\nError: Log directory %s needs to exist before calling bzz. Exiting...\n\n",mapArgs["-logdir"].c_str());
        return false;        
    }
...
/*    
    ...
    {
        fprintf(stderr,"\nError: Data directory %s needs to exist before calling _ebzz_ebzzd. Exiting...\n\n",mapArgs["-datadir"].c_str());
        delete mc_gState;
        return false;        
    }
*/
        ...
        (mc_gState->m_Params->NetworkName() == NULL))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  bzz <blockchain-name> [options]                     " + _("Start bzz Core Daemon") + "\n";

            strUsage += "\n" + HelpMessage(HMM__ebzz_ebzzD);                       // MCHN-TODO edit help message
        }
...
fprintf(stdout, "██████╔╝███████╗███████╗      ███████║╚███╔███╔╝██║  ██║██║  ██║██║ ╚═╝ ██║\n");
fprintf(stdout, "╚═════╝ ╚══════╝╚══════╝      ╚══════╝ ╚══╝╚══╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝\n");                                                                                                                         
                fprintf(stdout, " -----》Powered by WWW.BZZ.CEO》 Starting up node...\n\n");
            }
...
//                delete mc_gState;
    ...
    if(err)
    {
        ...
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Please restart bzz with reindex=1.\n",mc_gState->m_Params->NetworkName());                        
        }
        else
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Probably bzz for this blockchain is already running. Exiting...\n",mc_gState->m_Params->NetworkName());
        }
        delete pEF;
        delete mc_gState;                
        return false;
    ...
    if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
    {
        ...
        if(mc_gState->IsSupported(protocol_version) == 0) 
        {
            ...
            {
                fprintf(stderr,"ERROR: The protocol version (%d) for blockchain %s has been deprecated and was last supported in bzz %s\n\n",
            ...
            {
                fprintf(stderr,"ERROR: Parameter set for blockchain %s was generated by bzz running newer protocol version (%d)\n\n",
                        mc_gState->m_Params->NetworkName(),protocol_version);                        
                fprintf(stderr,"Please upgrade bzz\n\n");
                delete pEF;
                delete mc_gState;                
                return false;
            }
    ...
    switch(mc_gState->m_NetworkParams->m_Status)
    {
        ...
        case MC_PRM_STATUS_MINIMAL:
            if(mc_gState->GetSeedNode() == NULL)
            {
                ...
                fprintf(stderr,"Operation instance:_ebzz-bzzd bzz@ip\n\n");
               // fprintf(stderr,"  _ebzz_ebzz-util create %s\n",mc_gState->m_Params->NetworkName());
               // fprintf(stderr,"  _ebzz_ebzz-util clone <old-blockchain-name> %s\n",mc_gState->m_Params->NetworkName());
               // fprintf(stderr,"\nAnd rerun _ebzz_ebzzd %s\n\n\n",mc_gState->m_Params->NetworkName());                        
                // fprintf(stderr,"If you want to connect to existing blockchain please specify seed node:\n\n");
               // fprintf(stderr,"  _ebzz_ebzzd %s@<seed-node-ip>\n",mc_gState->m_Params->NetworkName());
               // fprintf(stderr,"  _ebzz_ebzzd %s@<seed-node-ip>:<seed-node-port>\n\n\n",mc_gState->m_Params->NetworkName());
                delete pEF;
                delete mc_gState;                
                return false;
            }
        ...
        case MC_PRM_STATUS_ERROR:
            ...
            fprintf(stderr,"  _ebzz-bzzd create %s\n",mc_gState->m_Params->NetworkName());
            fprintf(stderr,"  _ebzz-bzzd clone <old-blockchain-name> %s\n",mc_gState->m_Params->NetworkName());
            fprintf(stderr,"\nAnd rerun bzzd %s\n",mc_gState->m_Params->NetworkName());                        
            delete pEF;
            delete mc_gState;                
            return false;
        case MC_PRM_STATUS_INVALID:
            ...
            fprintf(stderr,"  _ebzz-bzzd clone %s <new-blockchain-name>\n",mc_gState->m_Params->NetworkName());
            delete pEF;
            delete mc_gState;                
            return false;
...
/*        
        ...
        if (fDaemon)
        {
            fprintf(stdout, "_ebzz_ebzz server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect _ebzz_ebzzd signal handlers
    noui_connect();
