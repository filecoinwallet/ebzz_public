...
int main(int argc, char* argv[])
{
    ...
    mc__ebzz_ebzzParams* params;
    mc__ebzz_ebzzParams* paramsOld;
    ...
    if(mc_gState->m_Params->Command())
    {
        if(strcmp(mc_gState->m_Params->Command(),"create") == 0)
        {
            ...
            {
                params=new mc__ebzz_ebzzParams;
...
//                    err=params->Create(mc_gState->m_Params->m_Arguments[1],version);
                ...
                { 
                    ...
                    printf("You can edit it in %s before running bzz for the first time.\n\n",fileName);
                    printf("To generate blockchain please run \"bzz %s -daemon\".\n",params->Name());
                }                
                else
                {
        ...
        if(strcmp(mc_gState->m_Params->Command(),"clone") == 0)
        {
            ...
            {
                params=new mc__ebzz_ebzzParams;
                paramsOld=new mc__ebzz_ebzzParams;
                ...
                if(err == MC_ERR_NOERROR)
                {
                    ...
                    printf("You can edit it in %s before running _ebzz_ebzzd for the first time.\n\n",fileName);
                    printf("To generate blockchain please run \"_ebzz_ebzzd %s -daemon\".\n",params->Name());
                }                
                else
                {
                    fprintf(stderr,"ERROR: Blockchain parameter set was not generated.\n");
...
*/        
    ...
    if(err == MC_ERR_OPERATION_NOT_SUPPORTED)
    {
        ...
        printf("Usage:\n");
        printf("  bzz create <blockchain-name>  ( <protocol-version> = %d ) [options]        Creates new bzz configuration file %s with default parameters\n",
        ...
        printf("  bzz clone <old-blockchain-name> <new-blockchain-name> [options]               Creates new bzz configuration file %s copying parameters\n",fileName);
