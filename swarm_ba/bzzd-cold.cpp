...
bool AppInit(int argc, char* argv[])
{
    ...
    {
        fprintf(stderr,"\nError: Data directory %s needs to exist before calling _ebzz_ebzzd-cold. Exiting...\n\n",mapArgs["-datadir"].c_str());
        return false;        
    }
        
    if(!mc_DoesParentLogDirExist())
    {
        fprintf(stderr,"\nError: Log directory %s needs to exist before calling _ebzz_ebzzd-cold. Exiting...\n\n",mapArgs["-logdir"].c_str());
        return false;        
    }
        ...
        (mc_gState->m_Params->NetworkName() == NULL))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  _ebzz_ebzzd-cold <blockchain-name> [options]                     " + _("Start _ebzz_ebzz Offline Daemon") + "\n";
    ...
    if(err)
    {
        fprintf(stderr,"ERROR: Couldn't read configuration file for blockchain %s. Please try upgrading _ebzz_ebzz. Exiting...\n",mc_gState->m_Params->NetworkName());
        delete mc_gState;                
        return false;
    }
    ...
    if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
    {
        ...
        if(mc_gState->IsSupported(protocol_version) == 0) 
        {
            ...
            {
                fprintf(stderr,"ERROR: The protocol version (%d) for blockchain %s has been deprecated and was last supported in _ebzz_ebzz %s\n\n",
            ...
            {
                fprintf(stderr,"ERROR: Parameter set for blockchain %s was generated by _ebzz_ebzz running newer protocol version (%d)\n\n",
                        mc_gState->m_Params->NetworkName(),protocol_version);                        
                fprintf(stderr,"Please upgrade _ebzz_ebzz\n\n");
                delete mc_gState;                
                return false;
            }
        }
