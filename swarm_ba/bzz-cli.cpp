...
std::string HelpMessageCli()
{
    ...
    strUsage += "  -cold                    " + _("Connect to bzzd: use bzzd default directory if -datadir is not set") + "\n";
/* MCHN START */    
    strUsage += "  -requestout=<requestout> " + _("Send request to stderr, stdout or null (not print it at all), default stderr") + "\n"; 
    strUsage += "  -saveclilog=<n>          " + _("If <n>=0 _ebzzbzz-cli history is not saved, default 1") + "\n";
/*    
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/_ebzz_ebzz.conf are parsed in qt/_ebzz_ebzz.cpp's main()
...
//    ParseParameters(argc, argv);
      ...
      {
        fprintf(stdout,"\n_ebzz_bzz:::::Console Mine V1.0.0 (Release) \n\n");
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "-------z---bb--  _ebzzbzz-cli BZZ [options] <command> [params]  " + _("Send command to _ebzz") + "\n" +
                  "-----zbbb--zbz-  _ebzzbzz-cli BZZ [options] help                " + _("List commands") + "\n" +
                  "----bbbbbz--z--  _ebzzbzz-cli BZZ [options] getaddresses                " + _("The address listed") + "\n" +
                  "----bbbbbbb----  _ebzzbzz-cli BZZ [options] getblock                " + _("Check the block information") + "\n" +
                  "-----zbbbzz----  _ebzzbzz-cli BZZ [options] issue                " + _("List permissions") + "\n" +
                  "-zbbbz---zzbbb-  _ebzzbzz-cli BZZ [options] mine                " + _("Open to dig") + "\n" +
                  "bbbbbbb-bbbbbbb  _ebzzbzz-cli BZZ [options] listaddress                " + _("Monitor the network") + "\n" +
                  "bbbbbbb-bbbbbbb  _ebzzbzz-cli BZZ [options] grant                " + _("Transfer transaction") + "\n" +
                  "--zbbz---zzbbz-  _ebzzbzz-cli BZZ [options] listpermissions <command>      " + _("list permissions") + "\n";
                  "bbbbbbb-bbbbbbb  _ebzzbzz-cli BZZ [options] mine <command>      " + _("Open to dig") + "\n";
...
*/
        ...
        else
        {
            ...
            if(read_err)
            {
                ...
                if (mapArgs["-rpcuser"] == "" && mapArgs["-rpcpassword"] == "")
                {
                    ...
                    {
                        fprintf(stderr,"ERROR: Couldn't read configuration file for blockchain %s. Please try upgrading bzz. Exiting...\n",mc_gState->m_Params->NetworkName());
                        return EXIT_FAILURE;
                    }
                }
            }
    ...
    Select_ebzz_ebzzBaseParams(mc_gState->m_Params->NetworkName(),RPCPort);
...
Object CallRPC(const string& strMethod, const Array& params)
{
    if (mapArgs["-rpcuser"] == "" && mapArgs["-rpcpassword"] == "")
        throw runtime_error(strprintf(
            _("Did not find BZZ configuration file \"%s\"\n\n"
                ...
                if (error.type() != null_type) {
                    ...
                    if (error.type() == obj_type)
                    {
                        ...
                        if (errMsg.type() == str_type)
                            strPrint += "bzz-_ebzz error message:\n"+errMsg.get_str();
                    }
...
int main(int argc, char* argv[])
{
   ...
   fprintf(stdout,"\n_ebzz_bzz:::::Console Mine V1.0.0 (Release) \n\n");
        ...
        fprintf(stdout," _______  _     _  _______  ______    __   __    _______  _______  _______    _______  _______ \n");
        fprintf(stdout,"|       || | _ | ||   _   ||    _ |  |  |_|  |  |  _    ||       ||       |  |       ||       |\n");
        fprintf(stdout,"|  _____|| || || ||  |_|  ||   | ||  |       |  | |_|   ||____   ||____   |  |   _   ||  _____|\n");
        fprintf(stdout,"| |_____ |       ||       ||   |_||_ |       |  |       | ____|  | ____|  |  |  | |  || |_____ \n");
        fprintf(stdout,"|_____  ||       ||       ||    __  ||       |  |  _   | | ______|| ______|  |  |_|  ||_____  |\n");
        fprintf(stdout," _____| ||   _   ||   _   ||   |  | || ||_|| |  | |_|   || |_____ | |_____   |       | _____| |\n");
        fprintf(stdout,"|_______||__| |__||__| |__||___|  |_||_|   |_|  |_______||_______||_______|  |_______||_______|\n");
        fprintf(stdout,"Open source code:www.bzz.bet::-> V1.0.0 (Release) os->->=======================================\n\n");
    

        mc_TerminalInput *term=new mc_TerminalInput;
        term->SetPrompt("linux@_ebzzBzz::OS->>");
        ...
        while(!exitnow && ((command=term->GetLine()) != NULL))
        {
            if((strcmp(command,"exit")==0) || 
               ...
               (strcmp(command,"lijia")==0))
            {
                printf("\nBye_bzz::The current just quit the CLI system, main process is still in operation. As the main process network is not stable\n");
                exitnow=1;    
            }
            else
            {
                ...
                offset=0;
                argc_p=0;
                strcpy(dest+offset,"_ebzzbzz-cli");
