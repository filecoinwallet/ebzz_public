...
int64_t _ebzz_ebzzTransaction_OffchainFee(int64_t total_offchain_size);
...
//        printf("P (%s,%d) (%s,%d,%d,%d,%d)\n",out.coin.m_OutPoint.hash.ToString().c_str(),out.i,C_ebzz_ebzzAddress(dest).ToString().c_str(),amounts->GetCount(),*required,*txout_with_inline_data,*is_empty);
...
//        printf("S (%s,%d) (%s,%d,%d,%d,%d)\n",hash.ToString().c_str(),n,C_ebzz_ebzzAddress(dest).ToString().c_str(),(int)it->second.m_CSAssets.size(),required,txout_with_inline_data,is_empty);
    }
...
bool FindRelevantCoins(CWallet *lpWallet,                                       // IN  Wallet (to access grouping object)
                       ...
                       unsigned char *in_row,int in_size,                       // TMP temporary buffer row and its size
                       mc_Script *lpScript,                                     // TMP temporary _ebzz_ebzz script object
                       int *in_special_row,                                     // IN  Coordinates of special rows
                       ...
                       std::string& strFailReason)                              // OUT error message
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(custom_good_for_coin_selection(txout.scriptPubKey) &&
            ...
            Parse_ebzz_ebzzTxOutToBuffer(hash,txout,tmp_amounts,lpScript,&allowed,&required,mapSpecialEntity,strError)))
        {
                        ...
                        std::string& strFailReason)                             // OUT error message
{
    ...
    {       
        ...
        if( (count<max_inputs) && (out.nDepth >= min_conf) )
        {
            ...
            tmp_amounts->Clear();
            if(custom_good_for_coin_selection(txout.scriptPubKey) && 
                    Parse_ebzz_ebzzTxOutToBuffer(hash,txout,tmp_amounts,lpScript,&allowed,&required,strError))
            {
                txout_with_inline_data=false;
                if(check_for_inline_coins)
                {
    ...
    BOOST_FOREACH(const COutput& out, vCoins)                                   // Inputs
    {
        ...
        if(mc_GetABCoinQuantity(in_amounts->GetRow(in_special_row[1]),coin_id))
        {
            ...
            if(mc_GetABCoinQuantity(in_amounts->GetRow(in_special_row[0]),coin_id))
            {
                ...
                if(Parse_ebzz_ebzzTxOutToBuffer(hash,txout,tmp_amounts,lpScript,&allowed,&required,mapSpecialEntity,strError))
                {
                    for(int i=0;i<tmp_amounts->GetCount();i++)
                    {
                      ...
                      const CCoinControl* coinControl)
{
    ...
    if(( nTargetValue > 0) || (!found_selected) )
    {
        ...
        if(!lpWallet->Select_ebzz_ebzzCoins(nTargetValue,vCoins,in_map,in_amounts,in_special_row[0],in_asset_row,in_prefered_row,setAssetCoins, nAssetValueIn, coinControl))
        {
            return false;
        }                        
                              ...
                              std::string& strFailReason)                       // OUT error message
{
    ...
    if(MIN_OFFCHAIN_FEE)
    {
        ...
        {
            _ebzz_ebzzTransaction_SetTmpOutputScript(s.first);
            mc_gState->m_TmpScript->ExtractAndDeleteDataFormat(NULL,NULL,NULL,&total_offchain_size);            
        }
        mandatory_fee=_ebzz_ebzzTransaction_OffchainFee(total_offchain_size);
    }
...
{   
    ...
    BOOST_FOREACH (const PAIRTYPE(CScript, CAmount)& s, vecSend)
    {
        if (nValue < 0)                                                         // _ebzz_ebzz allows protocol zero-value outputs
        {
...
//    if (vecSend.empty() || nValue < 0)
    ...
    if(vecSend.size())
    {
        ...
        if( (addresses == NULL) || (addresses->size() != 1) )
        {
            BOOST_FOREACH (const PAIRTYPE(CScript, CAmount)& s, vecSend)            // Filling buffer with output amounts
            {
                ...
                if(!Parse_ebzz_ebzzTxOutToBuffer(hash,txout,out_amounts,lpScript,NULL,&this_required,&mapSpecialEntity,strFailReason))
                {
                    goto exitlbl;
                }
    ...
    else
    {
        ...
        if(!Parse_ebzz_ebzzTxOutToBuffer(hash,txout,out_amounts,lpScript,NULL,&required,strFailReason))
        {
            goto exitlbl;
        }        
    }
    ...
    last_time=this_time;
                                                                                ...
                                                                                // We need this as in Select_ebzz_ebzzCoinsMinConf coins will be shuffled
...
bool CWallet::Create_ebzz_ebzzTransaction(const vector<pair<CScript, CAmount> >& vecSend,
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl,
...
//    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)                          // Not supported, not used in this case
    ...
    if( (tmp_amounts != NULL) && (lpScript != NULL) )                       // Count assets in unspent coins
    {
        ...
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            Parse_ebzz_ebzzTxOutToBuffer(hash,txout,tmp_amounts,lpScript,NULL,NULL,strError);
        }
                ...
                if(!result)
                {
                    ...
                    if (it != addressesToOptimize.end())
                    {
                        if(txOutCounts[it->second] >= min_outputs)
                        {
                            ...
                            C_ebzz_ebzzAddress _ebzz_ebzz_address=C_ebzz_ebzzAddress(*lpKeyID);
                            if(lpKeyID)
                            {
                                CWalletTx wtx;
                                ...
                                if(result)
                                {
                                    LogPrintf("Combine transaction for address %s (%d inputs,%d outputs): %s; Time: %8.3fs\n",
                                            _ebzz_ebzz_address.ToString().c_str(), wtx.vin.size(),wtx.vout.size(),wtx.GetHash().GetHex().c_str(),mc_TimeNowAsDouble()-start_time);
                                    total-=txOutCounts[it->second];
