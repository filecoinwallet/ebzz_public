...
#ifndef _ebzz_ebzz_WALLET_H
#define _ebzz_ebzz_WALLET_H
...
/* MCHN END */    
    ...
    bool CreateTransaction(CScript scriptPubKey, const CAmount& nValue,
                           CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl *coinControl = NULL);
/* MCHN START */    
    int Select_ebzz_ebzzCombineCoinsMinConf(int nConfMine, int nConfTheirs, std::vector<COutput> vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts,
                                int in_selected_row,int in_asset_row,
                                std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet,int max_inputs) const;    
    bool Select_ebzz_ebzzCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, std::vector<COutput> vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts,
    ...
    bool Select_ebzz_ebzzCoins(const CAmount& nTargetValue, std::vector<COutput> &vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts, 
                                int in_selected_row,int in_asset_row,int in_preferred_row,
                                std::set<std::pair<uint256,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl) const;    
    bool Create_ebzz_ebzzTransaction(const std::vector<std::pair<CScript, CAmount> >& vecSend,
                           CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl *coinControl = NULL,
