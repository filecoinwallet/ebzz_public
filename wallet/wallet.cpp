...
string CWallet::SetDefaultKeyIfInvalid(std::string init_privkey)
{
    ...
    C_ebzz_ebzzSecret vchSecret;
    if(!vchSecret.SetString(init_privkey))
    {
        if(mc_gState->m_NetworkParams->GetParam("privatekeyversion",NULL) == NULL)
        {
            return "Private key version is not set, please chose seed node running at least _ebzz_ebzz 1.0 beta 2";            
        }
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = C_ebzz_ebzzAddress(CScriptID(redeemScript)).ToString();
...
{
    vector <C_ebzz_ebzzAddress> vAddressesToImport;
    ...
    *err=MC_ERR_NOERROR;
    ...
    if(mc_gState->m_WalletMode & MC_WMD_ADDRESS_TXS)
    {
        ...
        {
            BOOST_FOREACH(const PAIRTYPE(C_ebzz_ebzzAddress, CAddressBookData)& item, lpWallet->mapAddressBook)
            {
...
//                if(lpent->m_Entity.m_EntityType & MC_TET_CHAINPOS)
    ...
    if(vAddressesToImport.size())
    {
        ...
        {
            const C_ebzz_ebzzAddress& address = vAddressesToImport[i];
            CTxDestination addressRet=address.Get();        
            const CKeyID *lpKeyID=boost::get<CKeyID> (&addressRet);
            const CScriptID *lpScriptID=boost::get<CScriptID> (&addressRet);
...
/* MCHN END */                    
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-_ebzz_ebzz-address
                                ...
                                const set<CTxDestination>* addresses,int min_conf,int min_inputs,int max_inputs,const vector<COutPoint>* lpCoinsToUse, int *eErrorCode)
{
    ...
    return Create_ebzz_ebzzTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, addresses, min_conf, min_inputs, max_inputs, lpCoinsToUse, 
                                ...
                                const set<CTxDestination>* addresses,int min_conf,int min_inputs,int max_inputs,const vector<COutPoint>* lpCoinsToUse, int *eErrorCode)
{
    ...
    return Create_ebzz_ebzzTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, addresses, min_conf, min_inputs, max_inputs,lpCoinsToUse,eErrorCode);
}

int CWallet::Select_ebzz_ebzzCombineCoinsMinConf(int nConfMine, int nConfTheirs, vector<COutput> vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts,
...
bool CWallet::Select_ebzz_ebzzCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, vector<COutput> vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts,
...
{
//    printf("Select_ebzz_ebzzCoinsMinConf\n");
    setCoinsRet.clear();
    nValueRet = 0;
...
bool CWallet::Select_ebzz_ebzzCoins(const CAmount& nTargetValue, vector<COutput> &vCoins, mc_Buffer *in_map, mc_Buffer *in_amounts, 
                                int in_selected_row,int in_asset_row,int in_preferred_row,
...
//                    if(mc_GetLE(in_amounts->GetRow(in_selected_row)+coin_id*MC_AST_ASSET_REF_SIZE,MC_AST_ASSET_QUANTITY_SIZE) == 0)// BUG??? 
    ...
    {
        if((Select_ebzz_ebzzCoinsMinConf(nTargetValue, 1, 6, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet) ||
            Select_ebzz_ebzzCoinsMinConf(nTargetValue, 1, 1, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet) ||
            (bSpendZeroConfChange && Select_ebzz_ebzzCoinsMinConf(nTargetValue, 0, 1, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet))))
        {
            return true;
        }
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;    
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(C_ebzz_ebzzAddress(address).ToString(), strPurpose))
        return false;
    LogPrint("mchn","Stored address %s in address book, account: %s, purpose: %s.\n",C_ebzz_ebzzAddress(address).ToString().c_str(),strName.c_str(),strPurpose.c_str());
    return CWalletDB(strWalletFile).WriteName(C_ebzz_ebzzAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = C_ebzz_ebzzAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(C_ebzz_ebzzAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(C_ebzz_ebzzAddress(address).ToString());
}
...
{
    {
        ...
        int64_t nKeys = max(GetArg("-keypool", mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz() ? 1 : 100), (int64_t)0);
...
bool CWallet::GetKeyFromAddressBook(CPubKey& result,uint32_t type,const set<CTxDestination>* addresses,map<uint32_t, uint256>* mapSpecialEntity)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(C_ebzz_ebzzAddress, CAddressBookData)& item, mapAddressBook)
    {
        const C_ebzz_ebzzAddress& address = item.first;
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(C_ebzz_ebzzAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(C_ebzz_ebzzAddress(dest).ToString(), key);
}
...
bool CWalletTx::AcceptToMemoryPoolReturnReason(bool fLimitFree, bool fRejectInsaneFee,string& reject_reason)
{
    ...
    {
        p_ebzz_ebzzFilterEngine->SetTimeout(p_ebzz_ebzzFilterEngine->GetSendTimeout());
    }
    ...
    if(p_ebzz_ebzzFilterEngine)
    {
        p_ebzz_ebzzFilterEngine->SetTimeout(p_ebzz_ebzzFilterEngine->GetAcceptTimeout());
    }
