        ...
        if(lpent->m_EntityType & MC_TET_CHAINPOS)
        {
            switch(lpent->m_EntityType & MC_TET_TYPE_MASK)
            {
                ...
                case MC_TET_SCRIPT_ADDRESS:
                    if(C_ebzz_ebzzAddressFromTxEntity(address,lpent))
                    {
...
/*                        
                        else
                        {
                            ...
                            if(p_ebzz_ebzzFilterEngine->RunStreamFilters(tx,i,entity.m_EntityID,filter_block, filter_offset, 
    ...
    else
    {
        if(StoredTxDef.m_Flags & MC_SFL_NODATA)
        {
            if(StoredTxDef.m_Flags & MC_SFL_IS_ADDRESS)
            {
                ...
                {
                    ret=C_ebzz_ebzzAddress((CScriptID)subkey_hash).ToString();
                }
                else
                {
                    ret=C_ebzz_ebzzAddress((CKeyID)subkey_hash).ToString();                    
                }
            }
        }
        else
