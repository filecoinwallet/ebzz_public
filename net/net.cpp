...
uint64_t nLocal_ebzz_ebzzServices = 0;
...
/* MCHN START */
    ...
    nLocal_ebzz_ebzzServices |= pEF->NET_Services();
    if(mc_gState->m_NetworkParams->IsProtocol_ebzz_ebzz())
    {
        subver=FormatSubVersion("_ebzz_ebzz", mc_gState->GetProtocolVersion(), std::vector<string>());
    }
    PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou, addrMe,
                nLocalHostNonce, subver, nBestHeight, true, nLocal_ebzz_ebzzServices);
/* MCHN END */
}
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (_ebzz_ebzz users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "_ebzz_ebzz " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. bzz Core is probably already running."), addrBind.ToString());
        else
