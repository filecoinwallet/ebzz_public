...
int mc_GenerateConfFiles(const char *network_name)
{
    ...
    fileHan=mc_OpenFile(NULL,"_ebzzbzz",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(NULL,"_ebzzbzz",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);
    }
    if(fileHan)
    {
    ...
    fileHan=mc_OpenFile(network_name,"_ebzzbzz",".conf","r",MC_FOM_RELATIVE_TO_DATADIR | MC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=mc_OpenFile(network_name,"_ebzzbzz",".conf","w",MC_FOM_RELATIVE_TO_DATADIR);
        if(fileHan)
        {
            mc_RandomEncodedBase58String(usrename,32);


            fprintf(fileHan,"rpcuser=bzz_node_%s\n\n",usrename);
            mc_RandomEncodedBase58String(rpcpwd,32);
            fprintf(fileHan,"rpcpassword=%s\n",rpcpwd);//rpcallowip=1.2.3.4/24
            fprintf(fileHan,"rpcallowip=127.0.0.1\n");
...
int mc__ebzz_ebzzParams::Build(const unsigned char* pubkey, int pubkey_size) 
{
