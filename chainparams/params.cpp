...
int mc_One_ebzz_ebzzParam::IsRelevant(int version)
{
    int ret=1;
...
void mc__ebzz_ebzzParams::Destroy()
{
    if(m_lpData)
    {
        mc_Delete(m_lpData);
...
int64_t mc__ebzz_ebzzParams::GetInt64Param(const char *param)
{
    int size;
...
double mc__ebzz_ebzzParams::Int64ToDecimal(int64_t value)
{
    if(value < 0)
    {
...
int64_t mc__ebzz_ebzzParams::DecimalToInt64(double value)
{
    return (int64_t)(value*MC_PRM_DECIMAL_GRANULARITY+mc_gState->m_NetworkParams->ParamAccuracy());    
}

int mc__ebzz_ebzzParams::GetParamFromScript(char* script,int64_t *value,int *size)
{
    char *ptr;    
    ptr=script;
    ptr+=strlen(ptr)+1;
...
double mc__ebzz_ebzzParams::GetDoubleParam(const char *param)
{
...
void* mc__ebzz_ebzzParams::GetParam(const char *param,int* size)
{
    if(m_lpIndex == NULL)
    {
        return NULL;
...
int mc__ebzz_ebzzParams::IsParamUpgradeValueInRange(const mc_One_ebzz_ebzzParam *param,int version,int64_t value)
{
...
int mc__ebzz_ebzzParams::CanBeUpgradedByVersion(const char *param,int version,int size)
{
    if(m_lpIndex == NULL)
    {
...
int mc__ebzz_ebzzParams::SetParam(const char *param,const char* value,int size)
{
    int offset;
    if(m_lpIndex == NULL)
    {
...
int mc__ebzz_ebzzParams::SetParam(const char *param,int64_t value)
{
    int size;
    char buf[8];
    if(m_lpIndex == NULL)
...
void mc__ebzz_ebzzParams::Init()
{
    ...
    m_Count=sizeof(_ebzz_ebzzParamArray)/sizeof(mc_One_ebzz_ebzzParam);
    max_size=0;
    
    for(i=0;i<m_Count;i++)
    {
        size=0;
        switch((_ebzz_ebzzParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
        {
            case MC_PRM_BINARY  : size=(_ebzz_ebzzParamArray+i)->m_MaxStringSize;   break;
            case MC_PRM_STRING  : size=(_ebzz_ebzzParamArray+i)->m_MaxStringSize+1; break;
    ...
    m_lpParams=(mc_One_ebzz_ebzzParam*)mc_New(sizeof(_ebzz_ebzzParamArray));
    m_lpData=(char*)mc_New(max_size);
    m_lpCoord=(int*)mc_New(2*m_Count*sizeof(int));
    
    memcpy(m_lpParams,_ebzz_ebzzParamArray,sizeof(_ebzz_ebzzParamArray));
    for(i=0;i<m_Count;i++)
    {
...
int mc__ebzz_ebzzParams::Create(const char* name,int version)
{
    int size,offset,i,set;
    mc_One_ebzz_ebzzParam *param;
    char *ptrData;
    int num_sets;
...
double mc__ebzz_ebzzParams::ParamAccuracy()
{
    return 1./(double)MC_PRM_DECIMAL_GRANULARITY;
}


const mc_One_ebzz_ebzzParam *mc__ebzz_ebzzParams::FindParam(const char* param)
{
    int i;
    for(i=0;i<m_Count;i++)
    {
        if(strcmp((_ebzz_ebzzParamArray+i)->m_Name,param) == 0)
        {
            return _ebzz_ebzzParamArray+i;
        }        
    }    
    return NULL;
}

int mc__ebzz_ebzzParams::Read(const char* name)
{
    return Read(name,0,NULL,0);
}

int mc__ebzz_ebzzParams::Read(const char* name,int argc, char* argv[],int create_version)
{
    ...
    mc__ebzz_ebzzParams *lpDefaultParams;
    ...
    lpDefaultParams = new mc__ebzz_ebzzParams;
    ...
    for(i=0;i<m_Count;i++)
    {
        ...
        {        
            ...
            switch((_ebzz_ebzzParamArray+i)->m_Type & MC_PRM_DATA_TYPE_MASK)
            {
                ...
                case MC_PRM_INT32:
                    size=4;
                    if(((_ebzz_ebzzParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) == MC_PRM_USER)
                    {
                        if(atoll(ptr) > (_ebzz_ebzzParamArray+i)->m_MaxIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too high: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                        if(atoll(ptr) < (_ebzz_ebzzParamArray+i)->m_MinIntegerValue)
                        {
                            printf("Invalid parameter value for %s - too low: %s\n",param->m_DisplayName,ptr);                        
                            return MC_ERR_INVALID_PARAMETER_VALUE;                                                
                        }
                    }
                    if((_ebzz_ebzzParamArray+i)->m_Type & MC_PRM_DECIMAL)
                    {
                        double d=atof(ptr);
                        if(d >= 0)
                        {
        ...
        {
            if( ((((_ebzz_ebzzParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_CALCULATED) && 
               (((_ebzz_ebzzParamArray+i)->m_Type & MC_PRM_SOURCE_MASK) != MC_PRM_GENERATED)) ||
               (argc > 0) )     
            {
                strcpy(m_lpData+offset,param->m_Name);
                offset+=strlen(param->m_Name)+1;
...
int mc__ebzz_ebzzParams::Set(const char *name,const char *source,int source_size)
{
...
int mc__ebzz_ebzzParams::Clone(const char* name, mc__ebzz_ebzzParams* source)
{
    int err;
...
int mc__ebzz_ebzzParams::CalculateHash(unsigned char *hash)
{
    int i;
    int take_it;
    mc_SHA256 *hasher;
...
int mc__ebzz_ebzzParams::Validate()
{
    int i,size,offset;
    int isGenerated;
    int isMinimal;
    ...
    if(isValid)
    {
        ...
        else
        {
            ...
            if(strcmp((char*)protocol_name,"_ebzz_ebzz") == 0)
            {
...
int mc__ebzz_ebzzParams::Print(FILE* fileHan)
{
    int i,c,size;
    int version;
    int header_printed;
    ...
    fprintf(fileHan,"# ==== bzz configuration file ====\n\n");
    fprintf(fileHan,"# Created by bzz \n");
    ...
    switch(m_Status)
    {
        case MC_PRM_STATUS_EMPTY:
            ...
            fprintf(fileHan,"# To join network please run \"bzz %s@<seed-node-ip>[:<seed-node-port>]\".\n",Name());
            return MC_ERR_NOERROR;
        case MC_PRM_STATUS_ERROR:
            ...
            fprintf(fileHan,"# Please fix it and rerun bzz-util. \n");
            break;
        ...
        case MC_PRM_STATUS_GENERATED:
            ...
            fprintf(fileHan,"# To generate network please run \"bzz %s\".\n",Name());
            break;
        case MC_PRM_STATUS_VALID:
            fprintf(fileHan,"# This parameter set is VALID. \n");
            fprintf(fileHan,"# To join network please run \"bzz %s\".\n",Name());
            break;
    }
        ...
        while(i>=0)
        {
                    ...
                    ((m_lpParams+i)->IsRelevant(version) > 0))
            {
                ...
                if(header_printed == 0)
                {
                    ...
                    {
                        case MC_PRM_COMMENT:
                            fprintf(fileHan,"# The following parameters don't influence bzz network configuration. \n");
                        ...
                        case MC_PRM_USER:
                            if(m_Status == MC_PRM_STATUS_ERROR)
                            {
                                ...
                                if(Name())
                                {
                                    fprintf(fileHan,"# Please rerun \"bzz clone %s <new-network-name>\". \n",Name());
                                }
                            }
                            else
                            {
                                ...
                                {
                                    fprintf(fileHan,"# The following parameters can be edited before running bzz for this chain. \n");                                    
                                }
                                else
                                {
                                    fprintf(fileHan,"# The following parameters can only be edited if this file is a prototype of another configuration file. \n");
                                    fprintf(fileHan,"# Please run \"bzz clone %s <new-network-name>\" to generate new network. \n",Name());
                                }
                            }
                            break;
                        case MC_PRM_GENERATED:
                            fprintf(fileHan,"# The following parameters were generated by bzz.\n");
...
int mc__ebzz_ebzzParams::Write(int overwrite)
{
    FILE *fileHan;
    int create;
...
const char* mc__ebzz_ebzzParams::Name()
{
    return (char*)GetParam("chainname",NULL);
}

const unsigned char* mc__ebzz_ebzzParams::MessageStart()
{
    return (unsigned char*)GetParam("networkmessagestart",NULL);
}

const unsigned char* mc__ebzz_ebzzParams::DefaultMessageStart()
{
    return c_DefaultMessageStart;
}


const unsigned char* mc__ebzz_ebzzParams::AddressVersion()
{
    return (unsigned char*)GetParam("addresspubkeyhashversion",NULL);
}

const unsigned char* mc__ebzz_ebzzParams::AddressScriptVersion()
{
    return (unsigned char*)GetParam("addressscripthashversion",NULL);
}

const unsigned char* mc__ebzz_ebzzParams::AddressCheckumValue()
{
    return (unsigned char*)GetParam("addresschecksumvalue",NULL);
}


int mc__ebzz_ebzzParams::ProtocolVersion()
{
...
int mc__ebzz_ebzzParams::IsProtocol_ebzz_ebzz()
{
    return m_IsProtocol_ebzz_ebzz;
}
